...
**Шаг 7 -- это две галочки в списке зданий локации, а не чужое окно.**
`in_game/gui/filters/bag_wtp_filters.txt`: «Планируемые» (статический список
типов, ничего не читает) и «Из плана — сюда» (`_plan_builds` той локации,
которую записала **общая с `rgo_bonus_filter`** проба `bag_view_location`).
Галочки видны и в ванильной панели, и в чужой. **Две копии общей пробы возят
оба мода побайтово одинаковыми**, `check_script.py` за этим следит. **`root` в
фильтре — не сам объект** (прогон 2026-09-09): спрашивать неявный `this` **до**
смены скоупа, за скоуп — только литерал.
