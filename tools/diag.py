...
def digest(lines: list[str]) -> list[str]:
    ...
    **Он существует, потому что отчёт не для человека.** Владелец, 2026-09-02:
    ...
    # **Проход деревень: первый слот каждой сельской локации.** Он идёт до
    # лестницы, поэтому в общих числах его работа неотличима от её; эта строка
    # — единственное место, где видно, что цепочка отработала и на каком шаге
    # она встала.
    vill = first("WTP VILL")
    if vill:
        kinds = [(name, field(vill, name)) for name in
                 ("fishing_village", "forest_village", "farming_village",
                  "market_village")]
        got = [(n, v) for n, v in kinds if v]
        none = field(vill, "none")
        total = sum(v for _, v in got)
        out.append("Деревни в первом слоте: %s; всего %s"
                   % (", ".join("%s %d" % kv for kv in got) or "ни одной", total))
        if none:
            out.append("  без деревни осталось %s — цепочку остановил гейт "
                       "здания: столько сельских локаций не пустила ни одна из "
                       "четырёх" % plural(none, "локация", "локации", "локаций"))
        else:
            out.append("  без деревни не осталось ни одной сельской локации")
