...
def digest(lines: list[str]) -> list[str]:
    ...
    **Он существует, потому что отчёт не для человека.** Владелец, 2026-09-02:
    ...
    # **Шаг 8, автострой через Construction Manager.** Одна строка на весь путь:
    # есть ли CM, доходит ли месячный лист, сколько помечено и сколько встало в
    # очередь. Порядок листьев месячного пульса между модами не определён, и
    # `ran=1 armed>0 staged=0` — это его симптом, а не «нечего строить».
    cm = first("WTP CM ")
    if cm:
        present, ran = field(cm, "present"), field(cm, "ran")
        armed, staged = field(cm, "armed"), field(cm, "staged")
        marked, last = field(cm, "marked"), field(cm, "last")
        if not present:
            out.append("Автострой CM: Construction Manager не в игре — кнопки "
                       "автостроя не рисуются, и это правильно")
        elif not ran:
            out.append("Автострой CM: !! месячный лист не отработал ни разу — "
                       "проверять кнопки бессмысленно, пока это ноль")
        else:
            out.append("Автострой CM: помечено %s, в прошлый месяц взято %s, "
                       "поставлено в очередь %s%s"
                       % (plural(marked or 0, "локация", "локации", "локаций"),
                          plural(armed or 0, "локация", "локации", "локаций"),
                          plural(staged or 0, "домик", "домика", "домиков"),
                          "" if staged or not armed else
                          " — либо в помеченных локациях строить нечего, либо "
                          "очередь CM стёрлась после нас; это разные вещи и "
                          "различает их только порядок листьев"))
            if last is not None and last != staged:
                out.append("  прошлый месяц дал %s" % last)
    ...
    if opensw is not None:
        ...
        if capped:
            ...
            # **Долей теперь две, и печатать одну — врать.** С 2026-09-06 доля
            # считается по трём классам товара: у насытившейся стороны свой
    ...
    if laps:
        ...
        out.append("Раздача по уровням (сколько домиков лёг каждый круг): "
                   ...
                   + ("" if ran is None else "; кругов всего %d, уровень %s"
                      % (ran, field(pas, "level"))))
    # Насколько плотно стоят деревни. Их законно больше, чем локаций: локация
    # держит по одной каждого из четырёх видов.
    room = first("WTP ROOM")
    vill, rural = field(room, "villages"), field(room, "rural_locs")
    if vill is not None and rural:
        out.append("Деревень %d на %d сельских локаций — %.1f на локацию из "
                   "четырёх возможных видов." % (vill, rural, vill / rural))
