...
def _cmm_chinese_localization_lines() -> list[str]:
    values = {
        ...
        "eu5ab_regional_development__performance_throughput_warning_summary_desc": "效率优先会每天处理最多 30 个地点，且每轮新增不限。大量地点应用模板时可能降低游戏性能。",
        "eu5ab_regional_development__performance_throughput_warning_action_name": "卡顿时请更换预设或降低「每轮最多新增」（0 为无限）。",
        "eu5ab_regional_development__performance_throughput_warning_action_desc": "需要改善游戏性能时，请改用其他性能预设，或降低「每轮最多新增」的数值。0 表示不限。",
...
def _cmm_english_localization_values() -> dict[str, str]:
    return {
        ...
        "eu5ab_regional_development__performance_throughput_warning_action_name": "If the game slows down, change preset or cap additions (0 is unlimited).",
        "eu5ab_regional_development__performance_throughput_warning_action_desc": "To improve game performance, choose another performance preset or lower Maximum Additions per Run. Zero means unlimited.",
...
def render_localization(policies: list[Policy], catalog: BuildingCatalog) -> str:
    lines = [
        ...
        ' eu5ab_custom_empty_detail: "左侧选择一个自定义模板，或新建空白模板/使用推荐优先级的模板。"',
        ...
        ' eu5ab_building_rules_desc: "第一行按就业阶层筛选，第二行按解锁时代分组。优先级范围为 0.0–10.0；0 表示完全不建造。特殊页只显示当前国家在至少一个自有地点能主动建造的特殊建筑。"',
        ...
        ' eu5ab_clear_visible_priorities_tooltip: "立即把当前类别与时代筛选交集中所有可见建筑的优先级设为 0，并保存当前模板。隐藏的特殊建筑不会改变。"',
        ...
        ' eu5ab_diag_rgo_short_desc: "每个逐项检查地点只记入一个结果；淘汰项按最先未通过的门槛计数。"',
        ' eu5ab_diag_rgo_help: "这里显示上次月度检查中原产候选依次通过各道门槛的结果。利用率按实际原产工人除以当前已扩建等级计算；劳动力按新增一级所需的1,000个岗位，结合当前空闲劳工、允许从事原产的空闲奴隶，以及设定期限内可晋升为劳工的人口预测。"',
        ' eu5ab_diag_rgo_checked_label: "检查的原产地点"',
        ' eu5ab_diag_rgo_eligible_label: "通过全部门槛"',
        ...
        ' eu5ab_diag_candidates_help: "每个地点只保留按食物紧急层、「自动建造顺序」及实际评分最高的一个建筑或原产候选，再将不同地点的候选全局排序并显示前三名。普通建筑和原产都会显示期限内劳动力预测；原产评分还会包含关闭劳动力暂停时的缺员扣分。候选不足三个时，对应位置会明确显示为空槽。"',
        ...
        ' eu5ab_diag_candidate_empty_value: "#weak 空槽：没有候选项目#!"',
        ...
        ' eu5ab_diag_candidate_1_rgo_scores_value: "队列总分 [GetPlayer.MakeScope.GetVariable(\'eu5ab_diag_top_1_score\').GetValue|0] · 地点需求 [GetPlayer.MakeScope.GetVariable(\'eu5ab_diag_top_1_need\').GetValue|0]"',
        ' eu5ab_diag_candidate_2_rgo_scores_value: "队列总分 [GetPlayer.MakeScope.GetVariable(\'eu5ab_diag_top_2_score\').GetValue|0] · 地点需求 [GetPlayer.MakeScope.GetVariable(\'eu5ab_diag_top_2_need\').GetValue|0]"',
        ' eu5ab_diag_candidate_3_rgo_scores_value: "队列总分 [GetPlayer.MakeScope.GetVariable(\'eu5ab_diag_top_3_score\').GetValue|0] · 地点需求 [GetPlayer.MakeScope.GetVariable(\'eu5ab_diag_top_3_need\').GetValue|0]"',
...
def _english_diagnostics_localization_values() -> dict[str, str]:
    values = {
        ...
        "eu5ab_diag_rgo_short_desc": "Each checked location records exactly one result; a rejection is counted at the first gate it fails.",
        "eu5ab_diag_rgo_help": "Shows how RGO candidates passed through each gate during the previous monthly check. Utilization is actual RGO workers divided by the currently expanded level. Workforce forecasts the 1,000 jobs added by the next level from unemployed laborers, eligible unemployed slaves, and laborer promotions within the configured horizon.",
        "eu5ab_diag_rgo_checked_label": "RGO Locations Checked",
        "eu5ab_diag_rgo_eligible_label": "Passed Every Gate",
        ...
        "eu5ab_diag_candidates_help": "Each location retains only its highest-priority building or RGO candidate according to the food-emergency layer, Automated Build Order, and the candidate's actual score. Candidates from distinct locations are then sorted globally and the top three are shown. Ordinary buildings and RGOs both show the workforce forecast; an RGO score also includes its staffing penalty when workforce pausing is disabled. Any unfilled position is displayed as an empty slot.",
        ...
        "eu5ab_diag_candidate_empty_value": "#weak Empty slot: no candidate#!",
        ...
        "eu5ab_diag_candidate_1_rgo_scores_value": "Queue Total [GetPlayer.MakeScope.GetVariable('eu5ab_diag_top_1_score').GetValue|0] · Location Need [GetPlayer.MakeScope.GetVariable('eu5ab_diag_top_1_need').GetValue|0]",
        "eu5ab_diag_candidate_2_rgo_scores_value": "Queue Total [GetPlayer.MakeScope.GetVariable('eu5ab_diag_top_2_score').GetValue|0] · Location Need [GetPlayer.MakeScope.GetVariable('eu5ab_diag_top_2_need').GetValue|0]",
        "eu5ab_diag_candidate_3_rgo_scores_value": "Queue Total [GetPlayer.MakeScope.GetVariable('eu5ab_diag_top_3_score').GetValue|0] · Location Need [GetPlayer.MakeScope.GetVariable('eu5ab_diag_top_3_need').GetValue|0]",
