...
def render_scripted_effects(policies: list[Policy], catalog: BuildingCatalog, rules: AutomationRules) -> str:
    ...
    def add_slot_defaults(slot: int) -> None:
        chunks.extend([
            ...
            *(f"\tremove_variable = {_slot_var(slot, suffix)}" for suffix in LEGACY_TEMPLATE_RUNTIME_SUFFIXES),
            "\t# New-save format: one building-type-keyed map replaces 316 scalar checks.",
            "\tif = {",
    ...
    for building_id in building_ids:
        chunks.append(
            ...
            f"value = {rules.building_priority_for(building_id):g} }}"
        )
    chunks.extend(["}", ""])
...
def render_engine_queue_effects(
    ...
    chunks = [
        ...
        *(
        ...
        "\t\tif = {",
        "\t\t\tlimit = { scope:eu5ab_worker_merge_country = { NOT = { has_variable = eu5ab_diag_top_locked } } has_variable = eu5ab_worker_top_1_building }",
        "\t\t\tscope:eu5ab_worker_merge_country = {",
        "\t\t\t\tset_variable = eu5ab_diag_top_locked",
        *(
            line
            for rank in range(1, rules.cadence.candidates_per_location + 1)
            for line in (
                f"\t\t\t\tset_variable = {{ name = eu5ab_diag_top_{rank}_location value = scope:{CANDIDATE_LOCATION_SCOPE} }}",
                *(
                    f"\t\t\t\tset_variable = {{ name = eu5ab_diag_top_{rank}_{suffix} value = scope:{CANDIDATE_LOCATION_SCOPE}.var:eu5ab_worker_top_{rank}_{suffix} }}"
                    for suffix in WORKER_TOP_FIELD_SUFFIXES
                ),
            )
        ),
        ...
        *(
        ...
        "\t\towner = {",
        f"\t\t\tif = {{ limit = {{ NOT = {{ is_target_in_variable_list = {{ name = eu5ab_active_project_locations target = scope:{CANDIDATE_LOCATION_SCOPE} }} }} }} add_to_variable_list = {{ name = eu5ab_active_project_locations target = scope:{CANDIDATE_LOCATION_SCOPE} }} }}",
...
def render_needs_scripted_effects(
    ...
    run_lines = [
        ...
        "\tset_variable = { name = eu5ab_diag_top_1_score value = -1000000 }",
        "\tset_variable = { name = eu5ab_diag_top_2_score value = -1000000 }",
        "\tset_variable = { name = eu5ab_diag_top_3_score value = -1000000 }",
        "\tremove_variable = eu5ab_diag_top_locked",
    ...
    run_lines.extend([
        ...
        "\t\tset_variable = { name = eu5ab_diag_top_location_score value = -1000000 }",
    ...
    for bucket in range(1, 21):
        ...
        for emergency_condition in (
            ...
            scan_lines.extend([
                ...
                "\t\t\t\towner = {",
                f"\t\t\t\t\tif = {{ limit = {{ var:eu5ab_diag_top_location_score < {{ value = scope:{CANDIDATE_LOCATION_SCOPE}.var:eu5ab_cached_location_need_score }} }} remove_variable = eu5ab_diag_top_locked set_variable = {{ name = eu5ab_diag_top_location_score value = scope:{CANDIDATE_LOCATION_SCOPE}.var:eu5ab_cached_location_need_score }} set_variable = {{ name = eu5ab_diag_top_1_score value = -1000000 }} set_variable = {{ name = eu5ab_diag_top_2_score value = -1000000 }} set_variable = {{ name = eu5ab_diag_top_3_score value = -1000000 }} }}",
    ...
    scan_lines.extend([
        ...
        "\tremove_variable = eu5ab_scan_active",
        "\tremove_variable = eu5ab_scan_bucket_day",
        "\tremove_variable = eu5ab_diag_top_location_score",
    ...
    candidate_limit = rules.cadence.candidates_per_location

    def candidate_diagnostic_lines(score_value: str) -> list[str]:
        top_values = (
            ...
            ("labor_pop_type", f"scope:{CANDIDATE_BUILDING_SCOPE}.eu5ab_candidate_workforce_pop_type_code"),
            ...
            ("labor_source_types", f"scope:{CANDIDATE_BUILDING_SCOPE}.eu5ab_candidate_promotion_source_type_count"),
            ("labor_deadline", f"scope:{CANDIDATE_BUILDING_SCOPE}.eu5ab_current_job_fill_deadline_months"),
            ("labor_prediction_available", f"scope:{CANDIDATE_BUILDING_SCOPE}.eu5ab_workforce_prediction_available"),
            ("labor_result", f"scope:{CANDIDATE_BUILDING_SCOPE}.eu5ab_labor_result_code"),
            ("labor_penalty", f"scope:{CANDIDATE_BUILDING_SCOPE}.eu5ab_labor_risk_penalty"),
            ("native_method", f"scope:{CANDIDATE_BUILDING_SCOPE}.eu5ab_native_input_method_code"),
            ("native_coverage", f"scope:{CANDIDATE_BUILDING_SCOPE}.eu5ab_native_input_coverage_percent"),
            ("native_score", f"scope:{CANDIDATE_BUILDING_SCOPE}.eu5ab_native_input_fit_proxy_score"),
            ("reason", "8"),
        )
        lines = [
            f"\t\tscope:{CANDIDATE_LOCATION_SCOPE} = {{",
            "\t\t\tif = {",
            "\t\t\t\tlimit = { NOT = { has_variable = eu5ab_candidate_rank } }",
            "\t\t\t\tset_variable = { name = eu5ab_candidate_rank value = 0 }",
            "\t\t\t}",
            "\t\t\tchange_variable = { name = eu5ab_candidate_rank add = 1 }",
        ...
        for rank in range(1, rules.cadence.candidates_per_location + 1):
            lines.extend([
                "\t\t\tif = {",
                f"\t\t\t\tlimit = {{ var:eu5ab_candidate_rank = {rank} }}",
                *(
                    f"\t\t\t\tset_variable = {{ name = eu5ab_worker_top_{rank}_{suffix} value = {value} }}"
    ...
    def candidate_gate_lines(
        ...
        """Classify expensive gates inside the retained top-three iterator."""
        lines: list[str] = []
        for index, (trigger, counter, cooldown, reason_code) in enumerate(gates):
            ...
            for rank in range(1, rules.cadence.candidates_per_location + 1):
                lines.extend([
                    "\t\t\t\tif = {",
                    "\t\t\t\t\tlimit = {",
                    f"\t\t\t\t\t\tscope:{CANDIDATE_LOCATION_SCOPE}.var:eu5ab_candidate_rank = {rank}",
                    "\t\t\t\t\t}",
                    f"\t\t\t\t\tset_variable = {{ name = eu5ab_worker_top_{rank}_reason value = {reason_code} }}",
                    "\t\t\t\t}",
                ])
            lines.extend(["\t\t\t}", "\t\t}"])
        lines.extend([
    ...
    chunks = [
        ...
        *(
            ...
            for line in (
                ...
                f"\tis_shown = {{ exists = var:eu5ab_q_active NOT = {{ exists = var:eu5ab_q_fire }} var:eu5ab_q_phase = {phase} }}",
                "}",
                "",
            )
        ),
...
def render_scripted_guis(
    ...
    def add_value_adjust(gui_id: str, variable: str, default: int, delta: int, minimum: int, maximum: int) -> None:
        effect_lines = [
            f"\t\tif = {{ limit = {{ NOT = {{ has_variable = {variable} }} }} set_variable = {{ name = {variable} value = {default} }} }}",
            f"\t\tset_variable = {{ name = {variable} value = {{ value = var:{variable} add = {delta} min = {minimum} max = {maximum} }} }}",
            "\t\teu5ab_commit_active_template_editor = yes",
        ]
        if variable == _editor_var("annual_budget"):
            effect_lines.append("\t\teu5ab_refresh_active_template_budget = yes")
        add_gui(gui_id, effect_lines)
    ...
    add_gui("eu5ab_gui_new_player_template", [
        "\t\tremove_variable = eu5ab_template_slot_claimed",
        "\t\tremove_variable = eu5ab_custom_templates_empty",
        "\t\tremove_variable = eu5ab_active_preset_policy",
        *[
            ...
            for line in [
                ...
                f"\t\t\tset_variable = {{ name = eu5ab_active_template_slot value = {slot} }}",
                "\t\t\teu5ab_load_new_template_into_editor = yes",
    ...
    for slot in TEMPLATE_SLOTS:
        ...
        delete_lines = [
            ...
            "\t\t}",
            "\t\t# Reset the reusable slot through the shared editor path without duplicating hundreds of fields here.",
            "\t\teu5ab_load_new_template_into_editor = yes",
            f"\t\teu5ab_commit_template_editor_to_slot_{slot}_and_refresh_budget = yes",
    ...
    for building_id in _catalog_building_ids(catalog):
        ...
        add_value_adjust(f"eu5ab_gui_active_priority_dec_{building_id}", variable, 0, -0.1, 0, 10)
        add_value_adjust(f"eu5ab_gui_active_priority_inc_{building_id}", variable, 0, 0.1, 0, 10)
...
{_gui_ogas_card(TEMPLATE_ICON, "eu5ab_custom_tab", f'''						text_multi = {{ layoutpolicy_horizontal = expanding autoresize = yes text = "eu5ab_custom_empty_detail" }}
                        {new_template_buttons("eu5ab_gui_new_player_template", "eu5ab_new_template_plus_button", 190)}''')}
							}}"""
...
{custom_sidebar}
							...
							vbox = {{
								...
								ignoreinvisible = yes
                                {new_template_buttons("eu5ab_gui_new_player_template", "eu5ab_new_template_plus_button", sidebar_button_width)}
							}}
						}}
					}}
					widget = {{
...
def _gui_active_priority_row(building_id: str, catalog: BuildingCatalog) -> str:
    ...
    return f"""hbox = {{
		...
		button_regular = {{
			size = {{ 46 36 }}
			enabled = "{dec_enabled}"
			tooltip = "eu5ab_priority_decrease"
			text_single = {{ size = {{ 38 32 }} parentanchor = center align = center|nobaseline text = "eu5ab_priority_decrease" }}
			onclick = "[GetScriptedGui('eu5ab_gui_active_priority_dec_{building_id}').Execute(GuiScope.SetRoot(GetPlayer.MakeScope).End)]"
		}}
		text_single = {{
			size = {{ 52 36 }}
			autoresize = no
		...
		button_regular = {{
			size = {{ 46 36 }}
			enabled = "{inc_enabled}"
			tooltip = "eu5ab_priority_increase"
			text_single = {{ size = {{ 38 32 }} parentanchor = center align = center|nobaseline text = "eu5ab_priority_increase" }}
			onclick = "[GetScriptedGui('eu5ab_gui_active_priority_inc_{building_id}').Execute(GuiScope.SetRoot(GetPlayer.MakeScope).End)]"
		}}
	}}"""
...
{rows}
				...
				text_single = {{ layoutpolicy_horizontal = expanding size = {{ -1 28 }} align = center text = "eu5ab_priority_scale_hint" }}
			}}
"""
    return _gui_window(
...
def _render_rules_candidates_card() -> str:
    ...
    for rank in range(1, 4):
        candidate_visible = _rules_gui_when_available(
            _rules_gui_greater(f"eu5ab_diag_top_{rank}_score", -999999)
        )
        reason_rows = []
        ...
        candidate_widgets.append(
            f"""vbox = {{
				...
				{_gui_kv_row("eu5ab_diag_label_building", f"eu5ab_diag_candidate_{rank}_building_value")}
				{_gui_kv_row("eu5ab_diag_label_scores", f"eu5ab_diag_candidate_{rank}_scores_value")}
				{_gui_kv_row("eu5ab_diag_label_workforce", f"eu5ab_diag_candidate_{rank}_workforce_value")}
				{'\n'.join(reason_rows)}
			}}"""
        )
    no_candidates = _rules_gui_less("eu5ab_diag_top_1_score", -999999)
...
def _cmm_chinese_localization_lines() -> list[str]:
    values = {
        ...
        "eu5ab_regional_development_desc": "为全部内置模板与自定义模板设置共享的国家级运行规则。模板本身只保留启停、应用范围和建筑优先级。",
        ...
        "eu5ab_regional_development__performance__preset_desc": "选择一组扫描参数；手动修改高级设置后会自动切换为「自定义」。修改从下一轮扫描开始生效。",
        "eu5ab_regional_development__performance__advanced_name": "高级设置",
        "eu5ab_regional_development__performance__advanced_desc": "控制独立地点任务、每日工作量、本轮新增上限，以及候选项目充足时是否提前停止扫描。",
        "eu5ab_regional_development__general__limits_name": "总开关与同时建造上限",
        "eu5ab_regional_development__general__limits_desc": "控制本 Mod 是否运行，以及本 Mod 最多可同时安排多少个民用建造。",
        "eu5ab_regional_development__general__returns_name": "收益条件与紧急放宽",
        "eu5ab_regional_development__general__returns_desc": "选择普通生产建筑的收益要求，并设置食物、建造所需商品、军需品和战略投入品短缺时是否放宽收益条件。",
        "eu5ab_regional_development__finance__budget_name": "共享年度预算",
        "eu5ab_regional_development__finance__budget_desc": "所有内置模板、自定义模板、普通建筑和原产共用同一个国家年度预算池。",
        "eu5ab_regional_development__finance__market_name": "市场价格范围",
        "eu5ab_regional_development__finance__market_desc": "用默认价格百分比设置所有模板共用的最低和最高价格参考。",
        "eu5ab_regional_development__automation__safety_name": "建造安全规则",
        "eu5ab_regional_development__automation__safety_desc": "设置特殊建筑、上游补建、劳动力和投入品短缺的共享处理规则。",
        "eu5ab_regional_development__automation__rgo_name": "原产扩建",
        "eu5ab_regional_development__automation__rgo_desc": "设置全部模板共用的原产开关和最低利用率。原产地点按短缺、价格、利用率和战略需求自动排序。",
        "eu5ab_regional_development__automation__workforce_name": "劳动力预测",
        "eu5ab_regional_development__automation__workforce_desc": "设置所有模板共用的岗位填补预测期限。",
        "eu5ab_regional_development__automation__ranking_name": "建造顺序与本地原料",
        "eu5ab_regional_development__automation__ranking_desc": "设置升级、扩建、原产和新建的先后顺序，以及本地原料对同类项目排序的影响。",
        "eu5ab_regional_development__automation__candidate_priority_name": "自动建造顺序",
        "eu5ab_regional_development__automation__candidate_priority_desc": "拖动四种建造类型来调整顺序。系统会先尝试当前类型中所有能开工的项目，再查看下一类；某个项目不能开工时，会继续寻找同类项目。食物紧急时，产出食物的项目会优先处理，但仍按这里的顺序安排。",
        ...
        "eu5ab_regional_development__candidate_priority_i1_desc": "将现有旧式建筑替换为当前已解锁的最新一代建筑。",
        "eu5ab_regional_development__candidate_priority_i2_name": "自动扩建现有建筑",
        "eu5ab_regional_development__candidate_priority_i2_desc": "为当地已有的普通建筑增加一级，不包含升级替换和原产。",
        "eu5ab_regional_development__candidate_priority_i3_name": "自动扩建资源采集点",
        "eu5ab_regional_development__candidate_priority_i3_desc": "扩建当地原产资源采集点，仍受最低利用率、年度预算和储备金等规则限制。",
        ...
        "eu5ab_regional_development__enabled_desc": "关闭后保留模板与地点绑定，但不再执行月度自动建造。",
        "eu5ab_regional_development__monthly_build_hard_cap_name": "额外同时建造上限",
        "eu5ab_regional_development__monthly_build_hard_cap_desc": "设置在基础 1 项之外，本 Mod 还可同时进行多少个民用建造。0 代表总计 1 项，599 代表总计 600 项；手动建造、道路和其他 Mod 的项目不占此上限。",
        "eu5ab_regional_development__budget_mode_name": "年度预算模式",
        "eu5ab_regional_development__budget_mode_desc": "选择固定金额，或按设置当年的月总收入倍数建立共享年度预算池。",
        ...
        "eu5ab_regional_development__economic_metric_desc": "选择普通生产建筑开工前必须达到的收益标准。收入和利润要求每月数值为正；投资回报要求年化回报率至少为 5%（约 20 年回本）。无产出基础设施不受此项限制；原产仍按利用率、价格和战略需求判断。",
        "eu5ab_regional_development__economic_metric_option_1_name": "@income! 收入",
        "eu5ab_regional_development__economic_metric_option_1_desc": "仅当游戏预计每月收入为正时，才允许普通生产建筑开工。",
        "eu5ab_regional_development__economic_metric_option_2_name": "@wealth! 利润",
        "eu5ab_regional_development__economic_metric_option_2_desc": "仅当游戏预计每月总利润为正时，才允许普通生产建筑开工。",
        "eu5ab_regional_development__economic_metric_option_3_name": "@efficiency! 收入回报率",
        "eu5ab_regional_development__economic_metric_option_3_desc": "仅当预计收入达到至少 5% 的年化建造成本回报率时，才允许普通生产建筑开工。",
        "eu5ab_regional_development__economic_metric_option_4_name": "@efficiency! 利润回报率",
        "eu5ab_regional_development__economic_metric_option_4_desc": "仅当利润达到至少 5% 的年化建造成本回报率时，才允许普通生产建筑开工。",
        "eu5ab_regional_development__emergency_food_exhaustion_override_name": "预计食物耗尽：优先建造，不检查收益",
        "eu5ab_regional_development__emergency_food_exhaustion_override_desc": "市场预计食物储备将耗尽时，先安排产出食物的项目；普通食物建筑不再要求达到所选收益标准。这些项目仍按「自动建造顺序」安排，并继续遵守预算、国库储备、劳动力、投入品、建造所需商品和游戏自身的建造条件。",
        "eu5ab_regional_development__emergency_food_stockpile_override_name": "食物储备不高于 25%：优先建造，不检查收益",
        "eu5ab_regional_development__emergency_food_stockpile_override_desc": "市场食物储备不高于 25% 时，先安排产出食物的项目；普通食物建筑不再要求达到所选收益标准。这些项目仍按「自动建造顺序」安排，其他建造限制照常生效。",
        "eu5ab_regional_development__emergency_construction_goods_override_name": "建造所需商品供应低于 65%：不检查收益",
        "eu5ab_regional_development__emergency_construction_goods_override_desc": "某种核心建造所需商品的市场供应低于需求的 65% 时，生产该商品的建筑不再要求达到所选收益标准。其他建造限制照常生效。",
        "eu5ab_regional_development__emergency_wartime_military_override_name": "战时军需供应低于 65%：不检查收益",
        "eu5ab_regional_development__emergency_wartime_military_override_desc": "国家处于战争，且某种军需品的市场供应低于需求的 65% 时，生产该商品的建筑不再要求达到所选收益标准。其他建造限制照常生效。",
        "eu5ab_regional_development__emergency_strategic_input_override_name": "战略生产缺料：不检查上游收益",
        "eu5ab_regional_development__emergency_strategic_input_override_desc": "当上游原料供应低于需求的 65%，并已导致本国食物、建造所需商品或军需建筑停产时，生产这种原料的建筑不再要求达到所选收益标准。其他建造限制照常生效。",
        "eu5ab_regional_development__fixed_annual_budget_name": "固定年度预算",
        "eu5ab_regional_development__fixed_annual_budget_desc": "仅在固定预算模式下使用；所有模板共用该金额。",
        ...
        "eu5ab_regional_development__min_cash_reserve_desc": "项目开工后国库必须至少保留的金额。",
        ...
        "eu5ab_regional_development__price_min_desc": "低于默认价格这一百分比的产出通常视为供应过剩。",
        ...
        "eu5ab_regional_development__price_max_desc": "高于默认价格这一百分比的产出会获得高价排序加分。",
        ...
        "eu5ab_regional_development__allow_special_buildings_desc": "允许自动建造考虑特殊建筑；仍须满足游戏自身的建造条件。",
        "eu5ab_regional_development__auto_build_input_sources_name": "短缺时自动补建上游",
        "eu5ab_regional_development__auto_build_input_sources_desc": "计划中的建筑因投入品短缺而无法开工时，尝试建设模板允许的上游来源。",
        "eu5ab_regional_development__pause_low_workforce_name": "劳动力不足时暂停",
        "eu5ab_regional_development__pause_low_workforce_desc": "预计岗位无法在期限内填满时，不建造该项目。",
        "eu5ab_regional_development__stop_input_shortage_name": "关键投入品短缺时停止",
        "eu5ab_regional_development__stop_input_shortage_desc": "市场关键投入品供应不足时，不建造相关项目。",
        "eu5ab_regional_development__allow_rgo_name": "允许扩建原产",
        "eu5ab_regional_development__allow_rgo_desc": "允许全部模板考虑扩建原产资源采集点。",
        "eu5ab_regional_development__rgo_min_utilization_name": "原产最低利用率",
        "eu5ab_regional_development__rgo_min_utilization_desc": "低于该利用率的原产不会继续扩建。",
        ...
        "eu5ab_regional_development__job_fill_deadline_months_desc": "预测本地人口晋升并填补新岗位时使用的期限。",
        "eu5ab_regional_development__native_input_priority_name": "本地原料优先级",
        "eu5ab_regional_development__native_input_priority_desc": "0–10；数值越高，能使用本省原产资源的建筑越容易被优先建造。",
        "eu5ab_regional_development__performance_preset_name": "性能预设",
        "eu5ab_regional_development__performance_preset_desc": "预设会同步下方高级设置；选择「自定义」时保留当前数值。",
        "eu5ab_regional_development__performance_preset_option_1_name": "保守",
        "eu5ab_regional_development__performance_preset_option_1_desc": "独立地点任务开启；每日 10 个地点；每轮最多新增 30 个项目；候选充足后提前停止。",
        "eu5ab_regional_development__performance_preset_option_2_name": "平衡",
        "eu5ab_regional_development__performance_preset_option_2_desc": "独立地点任务开启；每日 20 个地点；每轮最多新增 50 个项目；候选充足后提前停止。",
        "eu5ab_regional_development__performance_preset_option_3_name": "效率优先",
        "eu5ab_regional_development__performance_preset_option_3_desc": "默认。独立地点任务开启；每日 30 个地点；每轮新增不限；候选充足后提前停止。",
        "eu5ab_regional_development__performance_preset_option_4_name": "自定义",
        "eu5ab_regional_development__performance_preset_option_4_desc": "使用当前高级设置。",
        "eu5ab_regional_development__parallel_location_scan_name": "独立地点任务",
        "eu5ab_regional_development__parallel_location_scan_desc": "把不同地点拆成独立任务交给事件系统处理；这不代表游戏会使用多个 CPU 线程。国家级候选归并、预算、诊断和开工仍按确定顺序执行。",
        "eu5ab_regional_development__daily_location_task_limit_name": "每日地点任务上限",
        "eu5ab_regional_development__daily_location_task_limit_desc": "每天最多派发多少个地点进行深度候选计算。范围 1–30。",
        "eu5ab_regional_development__max_additions_per_run_name": "每轮最多新增",
        "eu5ab_regional_development__max_additions_per_run_desc": "限制本轮预计新增项目数。范围 0–600；0 表示不限。",
        "eu5ab_regional_development__early_stop_when_candidates_sufficient_name": "候选充足后提前停止",
        "eu5ab_regional_development__early_stop_when_candidates_sufficient_desc": "串行归并的候选储备达到本轮预计新增量的两倍后，不再派发新的地点任务。",
        "eu5ab_diagnostics_cmm_hint": "所有模板共用的规则请在社区模组框架的「模组设置」中修改；这里显示最近一次自动建造结果。",
...
def _cmm_english_localization_values() -> dict[str, str]:
    return {
        ...
        "eu5ab_regional_development_desc": "Shared country-level runtime rules for every built-in and custom template. Templates retain only state, coverage, and building priorities.",
        ...
        "eu5ab_regional_development__performance__preset_desc": "Choose a scan profile. Manually changing an advanced setting switches the preset to Custom. Changes take effect when the next scan starts.",
        "eu5ab_regional_development__performance__advanced_name": "Advanced Settings",
        "eu5ab_regional_development__performance__advanced_desc": "Control independent location tasks, daily work, the per-run addition cap, and early stopping once the candidate reserve is sufficient.",
        ...
        "eu5ab_regional_development__automation__ranking_name": "Build Order & Local Inputs",
        "eu5ab_regional_development__automation__ranking_desc": "Set the order for upgrades, expansions, RGOs, and new buildings, plus how local inputs affect ranking within a build type.",
        ...
        "eu5ab_regional_development__economic_metric_option_3_desc": "Queue a regular production building only when predicted income provides at least a 5% annualized return on its construction cost.",
        "eu5ab_regional_development__economic_metric_option_4_name": "@efficiency! Return on Profit",
        "eu5ab_regional_development__economic_metric_option_4_desc": "Queue a regular production building only when profit provides at least a 5% annualized return on its construction cost.",
        ...
        "eu5ab_regional_development__performance_preset_option_1_desc": "Independent location tasks on; 10 locations per day; at most 30 additions per run; early stop on.",
        "eu5ab_regional_development__performance_preset_option_2_name": "Balanced",
        "eu5ab_regional_development__performance_preset_option_2_desc": "Independent location tasks on; 20 locations per day; at most 50 additions per run; early stop on.",
        "eu5ab_regional_development__performance_preset_option_3_name": "Maximum Throughput",
        "eu5ab_regional_development__performance_preset_option_3_desc": "Default. Independent location tasks on; 30 locations per day; unlimited additions per run; early stop on.",
        ...
        "eu5ab_regional_development__parallel_location_scan_name": "Independent Location Tasks",
        "eu5ab_regional_development__parallel_location_scan_desc": "Split location calculations into separate event tasks. This does not claim that the game uses multiple CPU threads. Country-level merging, diagnostics, budgets, and construction remain deterministic.",
        "eu5ab_regional_development__daily_location_task_limit_name": "Daily Location Task Limit",
        "eu5ab_regional_development__daily_location_task_limit_desc": "Maximum locations dispatched for deep candidate calculation each day, from 1 to 30.",
        "eu5ab_regional_development__max_additions_per_run_name": "Maximum Additions per Run",
        "eu5ab_regional_development__max_additions_per_run_desc": "Cap expected additions in one run from 0 to 600. Zero means unlimited.",
        "eu5ab_regional_development__early_stop_when_candidates_sufficient_name": "Stop Early When Candidates Are Sufficient",
        "eu5ab_regional_development__early_stop_when_candidates_sufficient_desc": "Stop dispatching new location tasks after the serially merged reserve reaches twice the expected additions for this run.",
        "eu5ab_diagnostics_cmm_hint": "Change rules shared by all templates in Community Mod Framework's Mod Settings. This page shows the latest automated construction result.",
        "CMM_NUMERIC_INCREASE_MAX": "[SelectLocalization(EqualTo_string(Scope.GetFlagName, 'eu5ab_regional_development__monthly_build_hard_cap'), 'eu5ab_cmm_shift_increase_10', SelectLocalization(Or(EqualTo_string(Scope.GetFlagName, 'eu5ab_regional_development__fixed_annual_budget'), EqualTo_string(Scope.GetFlagName, 'eu5ab_regional_development__min_cash_reserve')), 'eu5ab_cmm_shift_increase_1000', 'eu5ab_cmm_shift_increase_max'))]",
        "CMM_NUMERIC_DECREASE_MIN": "[SelectLocalization(EqualTo_string(Scope.GetFlagName, 'eu5ab_regional_development__monthly_build_hard_cap'), 'eu5ab_cmm_shift_decrease_10', SelectLocalization(Or(EqualTo_string(Scope.GetFlagName, 'eu5ab_regional_development__fixed_annual_budget'), EqualTo_string(Scope.GetFlagName, 'eu5ab_regional_development__min_cash_reserve')), 'eu5ab_cmm_shift_decrease_1000', 'eu5ab_cmm_shift_decrease_min'))]",
...
def render_localization(policies: list[Policy], catalog: BuildingCatalog) -> str:
    lines = [
        ...
        ' eu5ab_custom_empty_detail: "左侧选择一个自定义模板，或点击 + 新建模板。"',
        ' eu5ab_template_intro_title: "模板编辑"',
        ' eu5ab_automation_buildings_intro: "自定义模板可自由修改并应用到地点；内置预设提供只读的推荐规则，也可以复制为自定义模板后修改。"',
        ' eu5ab_player_templates_button: "玩家模板"',
        ' eu5ab_copy_preset_to_player_button: "复制为玩家模板"',
        ' eu5ab_delete_template_button: "删除模板"',
        ' eu5ab_delete_template_tooltip: "打开删除确认。确认后会删除这个自定义模板，并同步清除所有绑定到它的地点。"',
        ' eu5ab_delete_template_confirm_prompt: "确定删除当前模板？"',
        ' eu5ab_delete_template_confirm_button: "确认删除"',
        ' eu5ab_delete_template_confirm_tooltip: "永久删除当前模板及其全部地点绑定。此操作无法撤销。"',
        ...
        ' eu5ab_pause_template_tooltip: "临时暂停或恢复当前模板。暂停后会保留规则和地点绑定，但自动建造不会使用此模板。"',
        ' eu5ab_select_template_tooltip: "选择此模板并查看规则详情。"',
        ' eu5ab_preset_readonly_desc: "这是只读的内置预设。可以暂停、恢复和设置应用范围；如需修改建筑优先级，请先复制为自定义模板。"',
        ' eu5ab_template_name_pencil_hint: "模板名"',
        ' eu5ab_template_name_click_hint: "点击模板名可重命名"',
        ' eu5ab_template_name_click_tooltip: "输入当前模板的新名称。"',
        ' eu5ab_template_rename_desc: "输入当前模板的新名称。自定义名称只在本次游戏会话中显示；模板的其他设置会正常保存。"',
        ...
        ' eu5ab_target_location_desc: "清除和解耦会先打开地块选择器。应用模板请使用对应槽位卡片上的选择按钮。"',
        ' eu5ab_template_editor_desc: "在这里命名当前模板，并设置建筑优先级、价格区间、劳动力和投入品规则。所有修改都会立即保存。"',
        ' eu5ab_template_auto_save_hint: "修改会立即保存，关闭窗口即可返回。"',
        ' eu5ab_rules_tab_finance: "财政与建造"',
        ' eu5ab_rules_tab_automation: "劳动力与建造偏好"',
        ' eu5ab_rules_tab_diagnostics: "运行状态"',
        ' eu5ab_template_editor_sections_title: "编辑分类"',
        ' eu5ab_template_editor_sections_desc: "建筑列表和运行规则分别打开，长列表也不会挤出窗口。"',
        ' eu5ab_open_buildings_editor_button: "建筑规则"',
        ' eu5ab_open_rules_editor_button: "运行规则"',
        ' eu5ab_edit_template_button: "编辑"',
        ' eu5ab_copy_to_slot_button: "复制到此槽"',
        ...
        ' eu5ab_cash_help: "储备金是国库必须保留的最低余额，不会被单独扣除。若项目开工后国库余额会低于储备金，该项目就不会开工。例：国库 1050、储备金 1000、项目成本 100，此时不会开工。"',
        ...
        ' eu5ab_budget_help: "全部内置预设和自定义模板共用一个国家级年度预算池。可选择固定年度金额，或按月总收入 ×4、×6、×8 计算；收入联动额度在每年 1 月首次检查时确定，年内不会随收入变化。普通建筑按游戏计算的实际成本结算；原产按生成时从原版价格表校验的基础成本结算。只有项目真正进入建造队列后才会扣除预算。"',
        ...
        ' eu5ab_quota_help: "0 代表本 Mod 最多同时进行 1 个民用建造，599 代表最多 600 个。只有本 Mod 确认开工且仍在建造的项目会占用名额；手动建造、道路和其他 Mod 的项目不计入。每月检查时，本 Mod 只补足自己的剩余空位。实际开工数还会受到已应用模板的地点数量、年度预算、储备金、财政状况和建筑条件影响。每个地点同一时间最多由本 Mod 安排 1 个项目；原产没有独立的月度上限。"',
        ...
        ' eu5ab_building_rules_desc: "第一行按就业阶层筛选，第二行按解锁时代分组。优先级范围为 0.0–10.0，步长 0.1；0 表示完全不建造。特殊页只显示当前国家在至少一个自有地点能主动建造的特殊建筑。"',
        ' eu5ab_allow_button: "允许"',
        ' eu5ab_ban_button: "禁止"',
        ' eu5ab_priority_decrease: "−"',
        ' eu5ab_priority_increase: "+"',
        ' eu5ab_priority_scale_hint: "0.0 = 禁止；0.1–10.0 = 允许并按数值排序"',
        ...
        ' eu5ab_operating_rules_title: "运行规则"',
        ' eu5ab_operating_rules_short_desc: "设置特殊建筑、上游补建和投入品短缺规则。"',
        ' eu5ab_operating_rules_help: "允许特殊建筑后，项目仍必须满足游戏自身的建造条件。开启「缺少投入品时自动补建上游」后，计划中的普通建筑因缺料无法开工时，本 Mod 会在此模板已启用的建筑中寻找能缓解短缺的上游建筑。开启「关键投入品短缺时停止扩建」后，原料风险过高的项目不会开工。"',
        ...
        ' eu5ab_rgo_help: "「允许此模板扩建原产」决定这个模板是否会考虑原产项目；原产仍须满足游戏自身的建造条件、最低利用率、年度预算和国库储备。「自动建造顺序」决定何时检查原产；原产地点直接按原料短缺、价格、利用率和战略需求排序，不再设置额外权重或月度上限。最低利用率可以避免继续扩建已经缺少工人的原产。#R 重要：#! 启用本 Mod 的原产扩建后，请关闭游戏原版自动化面板中的「自动扩建原产」，不要同时开启两套自动化。"',
        ...
        ' eu5ab_job_fill_deadline_desc: "岗位填补期限是系统愿意等待新增岗位被填满的最长时间，可在 0—96 个月之间调整。0 个月表示只接受当前劳动力就能满足的项目。超过期限时，若开启「劳动力不足时暂停扩建」，项目不会开工；关闭后只会降低建造优先级。"',
        ' eu5ab_deadline_0: "0月"',
        ' eu5ab_deadline_3: "3月"',
        ' eu5ab_deadline_6: "6月"',
        ...
        ' eu5ab_prediction_unavailable_short_desc: "无法可靠预测未来晋升或迁移，因此按当前可用劳动力保守判断。"',
        ' eu5ab_workforce_help: "开启「劳动力不足时暂停扩建」后，预计无法在期限内填满岗位的项目不会开工；关闭后仍会降低其建造优先级。预测以当前自由劳动力为基础，再按地点与国家的阶层晋升速度、目标阶层的晋升系数和合格来源人口，估算未来 0—96 个月可增加的劳动力，并以实际来源人口为上限。它不会假设未来有人迁入，因此仍保持保守。"',
        ...
        ' eu5ab_price_section_desc: "价格按当前市场价占商品基础价格的百分比计算。产出价格低于最低值时，该生产建筑不会进入备选列表；高于最高值时，会获得短缺加分。没有产出商品的基础设施不受价格区间限制。"',
        ...
        ' eu5ab_prediction_diagnostics_title: "劳动力估算与运行状态"',
        ' eu5ab_prediction_section_body: "系统按照「自动建造顺序」依次检查升级、普通扩建、原产扩建和新建。当前类型还有能开工的项目时，不会提前跳到下一类。预计食物储备将耗尽或不高于 25% 时，产出食物的项目优先于其他项目；普通食物建筑也不再要求达到所选收益标准。建造所需商品、战时军需品或战略建筑上游原料严重短缺时，匹配的建筑同样可以放宽收益要求，但不会因此排到其他建造类型之前。年度预算、国库储备、建造所需商品、劳动力和游戏自身的建造条件始终有效。系统只考虑已经解锁的最新一代建筑；升级时只计算新增岗位。"',
        ' eu5ab_diagnostics_title: "运行状态"',
        ' eu5ab_diagnostics_snapshot_note: "设置会立即保存；运行数据会在每月检查完成后更新。"',
        ' eu5ab_diagnostics_snapshot_help: "设置修改会立即保存，但这里显示的是每月 22 日完成的上次检查结果，不是实时数据。页面会记录具体年月和检查日期。覆盖范围、队列或设置刚发生变化时，请等待下一次月度检查；检查完成后，额度、实际结果、失败原因和项目排名才会更新。"',
        ' eu5ab_diag_overview_title: "月度运行概况"',
        ' eu5ab_diag_quota_title: "本轮可新增项目"',
        ' eu5ab_diag_result_title: "新增项目"',
        ' eu5ab_diag_failure_title: "上次月度检查：失败统计"',
        ' eu5ab_diag_candidates_title: "优先级最高的三个项目"',
        ' eu5ab_diag_label_status: "当前运行状态"',
        ' eu5ab_diag_label_last_run: "上次运行日期"',
        ' eu5ab_diag_label_covered: "已应用模板地点"',
        ' eu5ab_diag_label_preliminary: "当前可安排地点"',
        ' eu5ab_diag_label_deep_scored: "已逐项检查地点"',
        ' eu5ab_diag_label_legal: "通过劳动力门槛的项目"',
        ' eu5ab_diag_label_staged_candidates: "进入引擎验证的候选"',
        ' eu5ab_diag_label_engine_probes: "实际引擎查询次数"',
        ' eu5ab_diag_label_queue_throttle: "同时建造空位"',
        ' eu5ab_diag_label_engine_queue: "本轮建造进度"',
        ' eu5ab_diag_label_queue_recoveries: "自动重试次数"',
        ...
        ' eu5ab_diag_state_ready: "状态：月度检查已执行。"',
        ...
        ' eu5ab_status_engine_queue_recovered: "#R 上次检查已中止并自动重置#!"',
        ...
        ' eu5ab_status_prediction_proxy: "#Y 近似劳动力估算#!"',
        ' eu5ab_diag_no_build_this_run: "#Y 本月未新增项目#!"',
        ' eu5ab_diag_candidates_not_scanned_full: "本轮同时建造名额已满，因此没有扫描新的候选项目；这不代表模板规则没有合格建筑。"',
        ' eu5ab_diag_no_ranked_candidates: "上次检查没有符合条件的项目；请结合运行状态和失败统计判断。"',
        ' eu5ab_diag_quota_short_desc: "显示本 Mod 当前在建、同时建造上限以及本轮最多可新增名额。"',
        ' eu5ab_diag_label_capacity_summary: "本 Mod 建造名额"',
        ...
        ' eu5ab_diag_label_expected_this_run: "本轮预计新增"',
        ...
        ' eu5ab_diag_label_actual_income: "预计每月收入"',
        ' eu5ab_diag_label_actual_profit: "预计每月利润"',
        ...
        ' eu5ab_diag_result_help: "预计食物储备将耗尽或不高于 25% 时，系统先处理产出食物的项目；其他时候按「自动建造顺序」逐类检查。普通建筑不符合游戏计算的成本、收益或建造条件时，系统会继续寻找同类项目。原产扩建会检查原版建造条件，并按生成时从原版价格表校验的基础成本预留预算；地点按原料短缺、价格、利用率和战略需求排列。只有项目真正进入建造队列后，才会记录成功、扣除预算、开始等待期并占用本 Mod 的同时建造名额。"',
        ' eu5ab_diag_failure_short_desc: "每次月度检查时清零，仅保留上次检查结果。"',
        ...
        ' eu5ab_diag_candidates_help: "这里显示上次月度检查中优先级最高的三个项目。每项列出地点、建筑、总分、需求分、经济分和未被选中的原因；没有项目时会直接说明。"',
        ...
        ' eu5ab_candidate_reason_ranked: "项目符合条件；系统先按「自动建造顺序」选择类型，再按需求分和其他排序条件比较同类项目"',
        ...
        ' eu5ab_template_locations_desc: "选择当前模板要应用到的地点、省份或地区。"',
        ' eu5ab_template_locations_short: "应用范围："',
        ' eu5ab_view_scope_button: "查看实际覆盖范围"',
        ' eu5ab_template_scope_desc: "显示当前自定义模板或内置预设的实际覆盖结果，按「地区 › 省份 › 地点」排列。"',
        ...
        ' eu5ab_scope_expand_province_tt: "展开该省份并显示当前模板覆盖的地点。"',
        ' eu5ab_scope_collapse_province_tt: "折叠该省份，停止创建其地点列表。"',
        ' eu5ab_scope_remove_location: "取消应用"',
        ' eu5ab_scope_remove_location_tt: "从此地点移除本 Mod 的模板和单独设置的规则，并立即刷新覆盖统计。"',
        ' eu5ab_scope_clear_all: "清空当前模板的所有地点"',
        ' eu5ab_scope_clear_all_tt: "移除当前窗口所显示的全部地点绑定及其本 Mod 单独设置；不会影响其他模板。"',
        ...
        ' eu5ab_template_conflict_replace_desc: "选择器只显示尚未绑定本 Mod 规则的范围；省份或地区内已有规则的地点会保留并隐藏。"',
        ...
        ' eu5ab_decouple_selected_location: "EU5AB：解耦地块"',
        ' eu5ab_decouple_selected_location_desc: "让所选地块不再跟随省份模板更新。"',
        ' eu5ab_clear_selected_location_policy: "EU5AB：清除地块政策"',
        ' eu5ab_clear_selected_location_policy_desc: "先选择一个地点，再移除该地点的本 Mod 模板、储备金、建筑规则和运行规则。"',
    ]
    ...
    chinese_overrides = {
        "eu5ab_template_rules_window_title": "运行诊断",
        "eu5ab_open_rules_editor_button": "运行诊断",
        "eu5ab_automation_buildings_tab_desc": "管理本 Mod 的模板启停、建筑优先级与应用范围。共享运行规则位于社区模组框架的模组设置。",
        "eu5ab_action_bar_tooltip": "打开区域发展模板，管理模板启停、建筑优先级、应用范围与运行诊断。",
        "eu5ab_template_editor_desc": "在这里命名当前模板并设置建筑优先级。共享预算、市场、劳动力和建造安全规则位于社区模组框架的模组设置。",
        "eu5ab_template_editor_sections_title": "模板内容",
        "eu5ab_template_editor_sections_desc": "模板只保存启停状态、应用范围和建筑优先级；运行诊断在单独窗口显示。",
        "eu5ab_preset_readonly_desc": "这是只读的内置预设。可以暂停、恢复和设置应用范围；如需修改建筑优先级，请先复制为自定义模板。所有模板共用的规则请在社区模组框架的「模组设置」中修改。",
        "eu5ab_pause_template_tooltip": "临时暂停或恢复当前模板。暂停会保留建筑优先级和地点绑定，但自动建造不会使用此模板。",
    }
    for slot in TEMPLATE_SLOTS:
        chinese_overrides[f"eu5ab_template_slot_{slot}_summary"] = (
            "这个模板单独保存启停状态、应用地点和建筑优先级。所有模板共用的规则请在社区模组框架的「模组设置」中修改；「运行状态」会显示最近一次自动建造结果。"
        )
...
def _english_localization_values() -> dict[str, str]:
    return {
        ...
        "eu5ab_template_rules_window_title": "Operating Rules",
        ...
        "eu5ab_automation_buildings_tab_desc": "Manage this Mod's development templates, coverage, treasury reserve, and automated construction rules.",
        "eu5ab_action_bar_name": "Advanced Auto Build",
        "eu5ab_action_bar_tooltip": "Open regional development templates, coverage, treasury reserve, and automated construction rules.",
        "eu5ab_action_bar_icon": "@production_panel!",
        "eu5ab_action_bar_color": "gold",
        ...
        "eu5ab_custom_empty_detail": "Select a custom template on the left, or create a new one.",
        ...
        "eu5ab_delete_template_tooltip": "Open the deletion confirmation. Confirming deletes this custom template and clears every location bound to it.",
        ...
        "eu5ab_target_location_desc": "Clear and decouple actions open the location picker. Use the selection buttons on a template card to apply a template.",
        ...
        "eu5ab_rules_tab_diagnostics": "Run Status",
        "eu5ab_template_editor_sections_title": "Rule Categories",
        "eu5ab_template_editor_sections_desc": "Building and operating rules open in separate windows so long lists remain usable.",
        "eu5ab_open_buildings_editor_button": "Building Rules",
        "eu5ab_open_rules_editor_button": "Operating Rules",
        "eu5ab_edit_template_button": "Edit",
        "eu5ab_copy_to_slot_button": "Copy to This Slot",
        ...
        "eu5ab_budget_help": "Every built-in preset and custom template shares one national annual pool. Choose a fixed annual amount or monthly total income multiplied by 4, 6, or 8. An income-linked amount is sampled during the first January check and remains stable for that year. Ordinary buildings use the game's actual cost; RGOs use the base cost verified against vanilla's price table at generation time. Budget is deducted only after a project enters the construction queue.",
...
def _english_rules_localization_values() -> dict[str, str]:
    return {
        ...
        "eu5ab_building_rules_desc": "Filter by workforce on the first row and unlock age on the second. Priorities range from 0.0 to 10.0 in steps of 0.1; 0 disables construction. The Special page only lists special buildings the country can actively build in at least one owned location.",
        "eu5ab_allow_button": "Allow",
        "eu5ab_ban_button": "Ban",
        "eu5ab_priority_decrease": "−",
        "eu5ab_priority_increase": "+",
        "eu5ab_priority_scale_hint": "0.0 = disabled; 0.1–10.0 = allowed and ranked by value",
        ...
        "eu5ab_prediction_unavailable_short_desc": "Uses a conservative estimate of currently available workers instead of inventing future monthly promotion or migration figures.",
        "eu5ab_workforce_help": "With workforce pausing enabled, a project does not start when its jobs are not expected to fill by the deadline; otherwise the same risk lowers its build priority. The 0-96 month forecast starts from currently available workers, then applies local and national promotion speed, the target pop type's promotion factor, and the eligible source-pop pool. The forecast is capped by that real source pool and assumes no future migration, so it remains conservative.",
...
def _english_diagnostics_localization_values() -> dict[str, str]:
    values = {
        ...
        "eu5ab_prediction_diagnostics_title": "Workforce Estimate & Run Status",
        ...
        "eu5ab_diagnostics_title": "Run Status",
        "eu5ab_diagnostics_snapshot_note": "Settings save immediately; run data updates after the monthly check.",
        "eu5ab_diagnostics_snapshot_help": "Settings save immediately, but this page shows the previous check completed on day 22 rather than live data. It records the exact year, month, and check date. After coverage, queue, or settings change, wait for the next monthly check to refresh allowances, results, failures, and project rankings.",
        "eu5ab_diag_overview_title": "Monthly Run Overview",
        "eu5ab_diag_quota_title": "Projects Available This Check",
        "eu5ab_diag_result_title": "Project Additions",
        "eu5ab_diag_failure_title": "Previous Monthly Check: Failures",
        "eu5ab_diag_candidates_title": "Top Three Projects",
        "eu5ab_diag_label_status": "Run Status",
        "eu5ab_diag_label_last_run": "Last Run",
        ...
        "eu5ab_diag_label_legal": "Projects Passing Workforce Gate",
        "eu5ab_diag_label_staged_candidates": "Candidates Sent to Engine Validation",
        "eu5ab_diag_label_engine_probes": "Actual Engine Queries",
        "eu5ab_diag_label_queue_throttle": "Concurrent Slots",
        "eu5ab_diag_label_engine_queue": "Current Build Check",
        "eu5ab_diag_label_queue_recoveries": "Automatic Retries",
        ...
        "eu5ab_status_engine_queue_recovered": "#R Previous check stopped and reset automatically#!",
        ...
        "eu5ab_status_prediction_proxy": "#Y Approximate workforce estimate#!",
        "eu5ab_diag_no_build_this_run": "#Y No confirmed queue addition#!",
        "eu5ab_diag_candidates_not_scanned_full": "The concurrent construction limit was full, so this check did not scan new candidates. This does not mean the template has no qualifying buildings.",
        "eu5ab_diag_no_ranked_candidates": "The previous check found no qualifying projects. Check the run status and failure counts for the limiting reason.",
        ...
        "eu5ab_diag_label_capacity_summary": "This Mod's Construction Capacity",
        ...
        "eu5ab_diag_label_expected_this_run": "Expected This Check",
        ...
        "eu5ab_diag_label_actual_income": "Expected Monthly Owner Tax",
        "eu5ab_diag_label_actual_profit": "Expected Monthly Profit",
        ...
        "eu5ab_diag_result_help": "When food is expected to run out or is at or below 25%, food projects are checked first. Otherwise, projects are checked by Automated Build Order. If an ordinary building does not meet the game's cost, return, or construction conditions, the Mod looks for another project of the same type. RGO expansion checks vanilla construction conditions and reserves the base cost verified against vanilla's price table at generation time; RGO locations are ranked by raw-material shortage, price, utilization, and strategic need. A project counts as successful and uses budget and one of this Mod's concurrent slots only after it actually enters the construction queue.",
        "eu5ab_diag_failure_short_desc": "Cleared at each monthly check; only the previous check is retained.",
        ...
        "eu5ab_diag_candidates_help": "Displays the three highest-priority qualifying projects from the previous monthly check. Each entry shows location, building, total score, need score, economic score, and why it was not selected. When no project exists, the page says so directly.",
        ...
        "eu5ab_diag_label_scores": "Scores",
        ...
        "eu5ab_candidate_reason_ranked": "This project qualifies; Automated Build Order chooses the type first, then need and the remaining slots for this check decide between projects of that type",
...
def _english_scope_localization_values() -> dict[str, str]:
    return {
        ...
        "eu5ab_decouple_button": "Decouple Location",
        ...
        "eu5ab_scope_collapse_province_tt": "Collapse this province and unload its location list.",
        ...
        "eu5ab_template_conflict_replace_desc": "The selector lists only scopes that are not already bound to this Mod's rules. Existing assignments inside a province or area remain unchanged and are hidden.",
        ...
        "eu5ab_decouple_selected_location": "This Mod: Decouple Location",
        "eu5ab_decouple_selected_location_desc": "Stop the selected location from following province-template updates.",
        "eu5ab_clear_selected_location_policy": "This Mod: Clear Location Policy",
        "eu5ab_clear_selected_location_policy_desc": "Select a location, then remove its template, treasury reserve, building rules, and operating rules from this Mod.",
    }
...
def render_english_localization(policies: list[Policy], catalog: BuildingCatalog) -> str:
    ...
    values.update({
        "eu5ab_template_rules_window_title": "Runtime Diagnostics",
        "eu5ab_open_rules_editor_button": "Runtime Diagnostics",
        "eu5ab_automation_buildings_tab_desc": "Manage template state, building priorities, and coverage. Shared runtime rules are in Community Mod Framework Mod Settings.",
        "eu5ab_action_bar_tooltip": "Open regional-development templates to manage state, building priorities, coverage, and runtime diagnostics.",
        ...
        "eu5ab_template_editor_sections_title": "Template Data",
        "eu5ab_template_editor_sections_desc": "Templates retain only state, coverage, and building priorities; runtime diagnostics open separately.",
    ...
    for slot in TEMPLATE_SLOTS:
        values.update({
            ...
            f"eu5ab_template_slot_{slot}_summary": "This template separately stores its enabled state, assigned locations, and building priorities. Change rules shared by all templates in Community Mod Framework's Mod Settings; Run Status shows the latest automated construction result.",
...
def generated_files(
    ...
    files = {
        ...
        ROOT / "in_game" / "localization" / "simp_chinese" / "eu5ab_l_simp_chinese.yml": render_localization(policies, catalog),
        ROOT / "main_menu" / "localization" / "simp_chinese" / "eu5ab_l_simp_chinese.yml": render_localization(policies, catalog),
        ROOT / "in_game" / "localization" / "english" / "eu5ab_l_english.yml": render_english_localization(policies, catalog),
        ROOT / "main_menu" / "localization" / "english" / "eu5ab_l_english.yml": render_english_localization(policies, catalog),
    }
