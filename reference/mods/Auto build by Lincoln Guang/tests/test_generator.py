...
class GeneratorTests(unittest.TestCase):
    ...
    def test_builtin_presets_are_maintained_separately_from_custom_names(self):
        ...
        blank_defaults = _blocks_for_token(
            effects,
            "eu5ab_template_slot_1_ensure_defaults = ",
        )[0]
        self.assertNotIn("key = building_type:city_walls", blank_defaults)
        self.assertIn(
    ...
    def test_metadata_declares_cmf_hard_dependency(self):
        ...
        self.assertEqual(payload["version"], "0.9.1 Beta")
        project = tomllib.loads((ROOT / "pyproject.toml").read_text(encoding="utf-8"))["project"]
        self.assertEqual(project["version"], "0.9.1b0")
    ...
    def test_main_window_is_overview_and_editor_is_separate(self):
        ...
        self.assertIn("eu5ab_new_template_plus_button", main_gui)
    ...
    def test_deleting_player_template_clears_its_settings_and_linked_locations(self):
        ...
        self.assertIn("eu5ab_load_new_template_into_editor = yes", delete)
        self.assertIn(
        ...
        reset = _blocks_for_token(
            effects, "eu5ab_load_new_template_into_editor = "
        )[0]
        self.assertNotIn("eu5ab_edit_budget_mode", reset)
        self.assertNotIn("eu5ab_edit_min_cash_reserve", reset)
        self.assertIn("name = eu5ab_edit_priority_building_granary value = 10", reset)
    ...
    def test_template_list_and_footer_controls_use_fixed_bottom_layout(self):
        ...
        new_template = main_gui.index('text = "eu5ab_new_template_plus_button"')
        ...
        self.assertLess(left_scrollbox, new_template)
        self.assertLess(new_template, detail_column)
        ...
        left_list = main_gui[left_scrollbox:new_template]
    ...
    def test_rules_window_is_diagnostics_only_and_cmm_owns_shared_rules(self):
        ...
        self.assertEqual(len(registered_settings), 27)
        ...
        gui_localization_keys = set(
            re.findall(r'(?:text|tooltip)\s*=\s*"(eu5ab_[A-Za-z0-9_]+)"', gui)
        )
    ...
    def test_building_rows_are_generated_once_for_active_template(self):
        gui = self.files[ROOT / "in_game" / "gui" / "eu5ab_template_buildings_window.gui"]
        self.assertEqual(
            gui.count("eu5ab_gui_active_priority_dec_"), self.building_count
        )
        self.assertEqual(
            gui.count("eu5ab_gui_active_priority_inc_"), self.building_count
        )
    ...
    def test_priority_is_zero_to_ten_in_tenths_without_old_state_migration(self):
        ...
        scripted_guis = self.files[ROOT / "in_game" / "common" / "scripted_guis" / "eu5ab_scripted_guis.txt"]
        self.assertIn("one building-type-keyed map replaces 316 scalar checks", effects)
        ...
        self.assertIn("add = -0.1 min = 0 max = 10", scripted_guis)
        self.assertIn("add = 0.1 min = 0 max = 10", scripted_guis)
    ...
    def test_copy_preset_materializes_an_independent_player_template(self):
        ...
        self.assertLess(
            new_template.index("eu5ab_load_new_template_into_editor = yes"),
            new_template.index(
                "eu5ab_commit_template_editor_to_slot_1_and_refresh_budget = yes"
            ),
        )
    ...
    def test_generated_size_regression_limits(self):
        ...
        scripted_guis = self.files[ROOT / "in_game" / "common" / "scripted_guis" / "eu5ab_scripted_guis.txt"]
        self.assertLess(
            len(buildings.encode("utf-8")), self.building_count * 3_000
        )
        self.assertLess(len(buildings.splitlines()), self.building_count * 60)
        self.assertLess(len(rules.encode("utf-8")), 250_000)
        self.assertLess(
            len(effects.encode("utf-8")), self.building_count * 19_000
        )
        self.assertLess(
            len(scripted_guis.encode("utf-8")), self.building_count * 2_500
        )
        self.assertLess(
