...
def render_menu(game: Game, ids) -> tuple[str, str]:
    ...
    # The recipes on offer for whichever good is selected. A recipe travels as the
    # flag naming its own localization key, so one value labels the row, tells the
    # ranking which recipe was ticked, and keys the ceiling -- with nothing to keep
    # in step between the three.
    out.append("# The recipes that make the chosen good, best output first.")
    out.append("# Scope: country")
    out.append("wtp_collect_recipes = {")
    out.append("\tclear_global_variable_list = wtp_pending_recipes")
    out.append("")
    for gi, good in enumerate(game.goods_produced):
        ...
        for name, _ in rows:
            out.append("\t\tadd_to_global_variable_list = { name = wtp_pending_recipes"
                       " target = flag:wtp_recipe_%s }" % name)
        out.append("\t}")
...
def render_selected(game: Game, ids) -> str:
    """Bonus and ceiling of a recipe, keyed on the flag that stands for it."""
    out = [BOM + HEADER, "#",
           "# A recipe is carried as the flag naming its localization key, so both",
           "# lookups key on `flag:wtp_recipe_<method>`.",
           "# Scope: province", ""]
    names = []
    seen = set()
    ...
    out.append("wtp_selected_bonus = {")
    out.append("\tvalue = 0")
    for index, name in enumerate(sorted(names)):
        keyword = "if" if index == 0 else "else_if"
        out.append("\t%s = { limit = { global_var:wtp_recipe = flag:wtp_recipe_%s }"
                   " value = wtp_bonus_%s }" % (keyword, name, name))
    out.append("}")
    out.append("")

    # Keyed on the row being written, for the ceiling column, which has to read
    # every recipe rather than only the ticked one.
    out.append("wtp_pending_ceiling = {")
    out.append("\tvalue = 0")
    for index, name in enumerate(sorted(names)):
        keyword = "if" if index == 0 else "else_if"
        out.append("\t%s = { limit = { scope:wtp_pending_recipe = flag:wtp_recipe_%s }"
                   " value = wtp_ceiling_%s }" % (keyword, name, name))
    out.append("}")
    out.append("")
    return "\n".join(out)
...
def main() -> int:
    ...
    if widest[0] > RECIPE_ROWS:
        print("%s has %d recipes and the list holds %d; raise RECIPE_ROWS and the "
              "item_count of the recipes list to match"
