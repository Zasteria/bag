...
- **Не проявились:** жёлтое число закрепления и выравнивание столбиков.
  Собранные файлы при этом верные. **«Правка не работает» и «запускал прошлую
  сборку» отсюда неразличимы**, а установка из `mods.bat` — известный молчаливый
  отказ, починенный и ни разу не подтверждённый на его машине.

**Поэтому построен штамп сборки, а не догадка.** `generate.py` считает sha256 по
всем собранным файлам мода и кладёт шесть знаков в `bag_wtp_build`; шапка
редактора их печатает. Хеш по содержимому, а не по коммиту: коммит менялся бы
каждый раз и `refresh.py` сообщал бы о движении, которого нет. **Теперь любой
его скриншот сам говорит, какая это сборка.**

**Одна правка по существу всё же нашлась:** строка пикера полагалась на
умолчание `ignoreinvisible`, которого из дерева не проверить. Игра сама пишет
`= no` пять раз — теперь и мы пишем.

**«Пересчитать» не существует, и мод врал об этом своим же текстом.** Он поправил
меня дважды, и был прав оба раза: кнопки называются **«План»** («Составить общий
план на сейчас») и **«На конец»**. При этом `bag_wtp_edit_no_plan` сам говорил
«Нажмите "Пересчитать" на странице мода» — то есть ошибка шла не только от меня.
Исправлено.

**И одна ошибка поймана до отправки.** Штамп сперва вставили как
`[bag_wtp_build]`; ключ включается как `$ключ$` (так игра делает 28 466 раз).
Проверку на «голые скобки» написать не вышло — игра сама пользуется
`[road_icon]`, `[accepted_culture]` и сотнями таких концептов, и первая версия
дала 11 ложных срабатываний на `ru_loc_fix`. Снята до коммита.
