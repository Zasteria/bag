...
**2026-09-07, сборка `aff13a` — резервация по стеснённости сработала на полную
и не дала ничего; прогон закрыл не её, а сам вопрос.** Северная Германия, 416
локаций, 1380 зданий на 1380 мест, кругов 87.

Правило отработало в полную силу: 12 товаров держат 458 локаций, `stone`
владеет **всеми 53** своими сельскими, `tar` 59, `livestock` 91. И не сдвинулось
ничего: `stone` 34 сельских домика как и было, `iron` 28, `fish` 18,
`naval_supplies` 21; **`salt` потерял два** (31 → 29). Правило снято тем же днём.

**Почему — арифметика самого отчёта, столбец `q − n`.** Девятнадцать товаров
кончили с `n` вплотную к своей квоте, то есть **остановлены квотой, а не
землёй**, и стоят, считая РГО, на 60–65: `beer` 61+0, `clay` 45+16, `livestock`
25+40 = 65 и `stone` **45+15 = 60**. То есть `stone` уже стоял на уровне со
всеми, а 19 «недобранных» сельских локаций ему просто не нужны — он выбрал свою
квоту. **Предсказание «stone 45 → 60» было невыполнимо: его квота кончилась на
47.** Посылка, из которой выросла резервация, считала неиспользованные комнаты
вместо достигнутого уровня, и это была ошибка чтения, а не плана.

**Ниже уровня остались четверо, и всем им не хватает земли, а не квоты:**
`salt` 53 (`q−n`=9), `iron` 51 (11), `fish` 39 (23), `naval_supplies` 38 (24).
`fish` и `naval_supplies` — та самая теснота побережья: 42 локации на двоих.
`iron` взял 40 из 43 возможных. **Чинить тут нечего**, и это ответ на всю линию
«равномерности».

**И один срез из того же отчёта, потому что вопрос «дефицитные получают всё, что
им нужно?» задаётся каждый раз.** Взято от своей земли, сельская сторона:

| товар | сельских мест | взял | РГО | квота на старте |
| --- | --- | --- | --- | --- |
| `iron` | 28 | **28, все** | 11 | 25 |
| `stone` | 53 | 34 | 15 | 21 |
| `salt` | 42 | 29 | 19 | 17 |
| `fish` | 42 | 18 | 21 | 15 |
| `livestock` | 137 | 25 | 40 | **1** |

**Железо берёт всю свою землю, и это ступени работают**: 25 ступенчатых кругов
идут раньше 5 общих в каждом круге, поэтому железо с выгодой **ноль** выбирает
раньше пива с выгодой 1000. **Недобирают соль и камень**, и у обоих стартовая
квота урезана своими же РГО. **Причина не названа** — назвал бы её один счётчик:
круг, на котором товар положил последний домик. Пока это подозрение.

**Разбор теперь считает сам `tools/diag.py`**: кого остановила квота, кого
потолок города, а кому не хватило земли.
