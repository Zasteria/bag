...
**2026-09-07, сборка `a23f65` — доля устоялась: одна квота на товар, два потолка
по сторонам. Прогона не было.**

Три версии за день, и последняя — его. Товар строит, пока он **под общей квотой**
(вся земля ÷ все товары — это равномерность) **и** под **потолком той стороны**,
куда встаёт. Потолки домиков не добавляют, они мешают одному товару выгрести
дефицитную сторону.

**Отвергнуты на числах, обе проверки его:** две независимые квоты по сторонам
(товар с дублем кончал вдвое жирнее односторонних — его собственная поимка) и
вес сельского домика по отношению выхлопов (стеклу нужно 80 сельских домиков при
42 сёлах; недостижимая квота не останавливает товар никогда).

**И выхлоп не различает стороны:** 59 городских методов дают меньше 1.0,
13 сельских — 1.0 и выше.

**Что проверять:** `WTP SHARE` — квота и оба потолка; `nt=`/`nr=` в строках
товаров.
...
`tools` и `weaponry` при `ng=48` кончили с нулём. Выгода 89 % зданий, в среднем
78.9 % от потолка рецепта (у равномерного плана на той же земле было 64 %).
...
**Общий план на северной Германии он посмотрел, но диагностику прислал только от
специализации** — оба отчёта в файле оказались одним и тем же нажатием (второй
обрезан). Его наблюдения глазами: городские товары встали на 14–15, сельские на
60–63, красители 13, рыба 18 сельских, железо 39, фрукты 7+41, инструменты и
клинки по 11+50 = 61. Ни одно из этих чисел не проверено логом.
