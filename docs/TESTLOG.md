...
**И его вопрос вскрыл настоящее.** «В одну локацию три отдельных торговых деревни
не встанет никогда.» Игра помечает четыре здания `is_village = yes`; план на этой
земле просит **328** деревень на **280** сельских локаций. Если деревня в локации
одна, план невозможен на 48 зданий. Построено выключателем «В локации одна
деревня», включён по умолчанию; отчёт теперь печатает
`ROOM … villages=N rural_locs=M`. Разбор —
