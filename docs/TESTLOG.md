...
стеснённых оказался пустым.** 1380 из 1380. `forest_village` 71 и
`market_village` 71 упёрлись в потолок, `fishing_village` 35. Рыба, пиво,
гончарка, инструменты, оружие и кожа — сельский счёт ноль, как и задумано.

**Железо 11+24 = 35 из 43, и причина числовая, а не в очереди.** Проход
стеснённых судил по `_plan_quota`, а она в тот момент **стартовая**: 1380 ÷ 38 =
36. Земля железа 43 больше 36 — «не стеснённый», проход его пропустил. К концу
доля доросла сухими кругами до 71, железо стало стеснённым (`qt=qr=60`, потолки
сторон сняты), но проход уже прошёл. **Всё, что считалось от доли до её роста,
считалось от вранья.**

**Доля стороны теперь считается точно и сразу.** Наименьшее X, при котором
`Σ min(своя земля, X)` накрывает комнаты стороны: земля заполнится ровно тогда.
Его же числа, угаданные по этому прогону, — «лимит в города 22–23, лимит в сёлах
47–48» — выходят из этой формулы, а не из сухих кругов.
...
**2026-09-08, четвёртая сборка — регрессия починена, земля снова 100 %, и он
поставил точку по деревням.** 1380 из 1380, `qt`/`qr` растут вместе с квотой
(город 10…62, село 52…70). Железо `qt=qr=62` — признак стеснённости работает, но
оно взяло 11+26 = 37 из своих 43: два города и две деревни успели забиться
другими, пока оно ждало своей очереди.

**Его вывод, и он верен:** пока деревенские товары считаются товарами, читать
эти числа нельзя. Инструменты 22+12, пиво 23+9 — двенадцать и девять сельских
домиков это торговые и лесные деревни, которых в счёте товара быть не должно.

**Сельский котёл построен тем же днём.** Деревня — сущность круга: своя доля,
свой уровень, своя ступень, свой выбор товара по лучшему из её рецептов. Товару
её домик **не засчитывается**, `_pn`/`_pnr` не растут. Делитель сельской доли —
**сущности, а не товары**: 14 товаров со своим сельским зданием плюс 3 деревни =
**18** вместо 23, то есть 852 ÷ 18 = 47 против 37.
