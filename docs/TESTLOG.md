...
**Ровность, считая РГО**, у 20 из 23 умеющих в село: **52…64**. Два исключения —
`fish` 17 + 21 РГО = 38 и `naval_supplies` 39, — и они об одном: **рыбу в селе
делает только рыбацкая деревня, и её же делает `naval_supplies`.** Прибрежных
локаций 42, рыбацких деревень построено 38 (17 + 21). Побережье использовано на
90 %; голодания там нет, и мой первый разбор («126 комнат, 88 у чужих») был
неверен — комната и здание не одно и то же.

**Исправлено в тот же день**: ступень дефицита считает места своей стороны
(`_ngt`/`_ngr`), а не обе разом — `naval_supplies` умеет 178 локаций всего и
потому проходил как массовый товар, имея в сёлах те же 42 места, что рыба.

**И один вопрос закрыт его же ответом.** «1 ТОРГОВАЯ деревня на 1 локацию, но
все 4 вида деревень в одной локации стоять могут.» Значит правило мода — запрет
на повтор **одного и того же здания** — было верным изначально; выстроенный на
полдня выключатель «В локации одна деревня» снят. От эпизода осталось число в
отчёте: `ROOM … villages=328 rural_locs=280`, то есть 1.2 деревни на локацию из
четырёх возможных видов.
