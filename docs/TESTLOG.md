...
числа сходятся до единицы.** 416 локаций, 74 провинции, 136 городов, 280 сёл;
`placed=1384 rooms=1384`, `laps=87` при пределе 150, `dry_laps=26`, доля 36,
городской потолок 15, сельский 36.

**Счёт закрывается ровно:** сумма `nt` по всем товарам = **544** (городских
комнат 136×4), сумма `nr` = **840** (сельских 280×3), сумма `n` = **1384**.
Отвечает на его подозрение, что деревни считаются по нескольку раз: не считаются,
одна деревня держит `cap_rural` = 3 здания разных товаров.
...
где-то касается 1000, против 12 у тех, кто не касается нигде. Теперь среди
**только городских** товаров все пятнадцать легли в **16…19** независимо от
выгоды: `incense` с лучшей выгодой 0 получил 19, `dyes` с 952 — 16. Разницу
между ними делает не выгода, а РГО (`dyes rgo=5`, `incense rgo=0`).

**Ровность, считая РГО**, у 20 из 23 умеющих в село: **52…64**. Три исключения —
все про побережье.

| товар | домиков | РГО | вместе | мест в сёлах |
| --- | --- | --- | --- | --- |
| fish | 17 | 21 | **38** | 42 |
| naval_supplies | 39 (18 город + 21 село) | 0 | **39** | 42 |
| остальные 20 | | | 52…64 | |

**Побережья 42 локации, в них 126 комнат; рыба и корабельные припасы держат из
них 38.** Остальные 88 достались товарам, которым подходят все 416 локаций.
Причина названа: `naval_supplies` умеет 178 локаций **всего**, поэтому ворота
ступени дефицита считали его массовым товаром — а в сёлах у него ровно те же 42
места, что у рыбы. **Исправлено в тот же день**: ступень считает места своей
стороны (`_ngt`/`_ngr`), а не обе разом. Прогона у правки нет.

**Не исправлено и требует его решения:** рыба стоит в 17 из 42 своих деревень, а
не в 42. Её доля была 36 − 21 РГО = **15** и держала её ровно тогда, когда
побережье разбирали; когда сухие круги долю подняли, свободных деревень уже не
было. Резервация по стеснённости («локация принадлежит самому стеснённому из
тех, кто в ней может») подняла бы рыбу и корабельные припасы до 42 каждый — то
есть ровно на общий уровень 60 — ценой ~46 комнат у товаров, у которых
альтернатив четыреста. Разбор — в
