...
**2026-09-07, сборка `a23f65` — доля свелась к одной квоте на товар и двум
потолкам по сторонам.** Прогона у неё не было: назавтра равномерность перешла
к уровню. Отвергнутые версии и числа, которыми их отвергли, —
[`archive/testlog_wtp_three_class_share.md`](archive/testlog_wtp_three_class_share.md).

**2026-09-06, две сборки трёхклассовой доли** — модель сошлась с игрой до
последней цифры, земля забита целиком, выгода 80 % при 71.6 % от потолка.
Схема снята 2026-09-07; записи целиком в
[`archive/testlog_wtp_three_class_share.md`](archive/testlog_wtp_three_class_share.md).
...
**2026-09-06, шаги 2–5 редактора приняты** — доля, перетасовка, меню, РГО,
выселение, журнал нажатий, грамоты. Записи целиком в
[`archive/testlog_wtp_steps.md`](archive/testlog_wtp_steps.md).

**2026-09-07, специализация на Вестфалии — принята как механизм, отвергнута как
выбор.** Первый прогон нового режима: 48 локаций, 174 здания в 174 местах,
`mode=1`, круги 48/36/19 (круг кладёт не больше одного домика в локацию, и 48 в
первом — это все локации разом). **Провинция действительно раздаёт грамоту всем
своим городам**: 7 городов Липпштадта взяли винокуренные, 7 городов Зауэрланда
корабельные, 3 города Оснабрюка книгопечатные. И **средняя по провинции
считается на самом деле**, а не подменяется числом первого города: в провинции 3
Линген оценил корабельные в 1000 против книгопечатных 802, а выдали
книгопечатные — потому что в двух других городах корабельные стоят 500, и
средняя за них ниже. Форма без прецедента
(`change_global_variable = { add = bag_wtp_rq<k> }` по локационной script value
внутри обхода по локациям) **работает**.

**Цена режима:** 25 товаров из 47 получили хоть что-то, 22 — ничего; `tools` и
`weaponry` при `ng=48` кончили с нулём. Выгода 89 % зданий при 78.9 % от потолка.

**И владелец отверг выбор, назвав причину.** Зауэрланд взял корабельные (1000)
вместо инструментальных (799): «для суд материалов есть целых 2 провинции,
которые дают ей бонусы, а для инструментов — только 1 — зауэрлэнд… редкий товар =
востребованный товар = ценный товар». Числа его подтверждают: корабельные берут
1000 в двух провинциях из восьми, инструментальные 799 в одной и 200 в двух.
Разбор и предложенное правило —
