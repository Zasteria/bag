...
## Автостройка CM: флажок на локации есть не у того, у чего кажется

Измерено на CM 2.2.12, 2026-09-04, под вопрос «наш мод ставит флажки
автостроительства пачкой, дальше CM сам».

**Пофлажковая модель верна для еды и RGO, и неверна для зданий.**

| что | как хранится | пишется ли чужим модом |
| --- | --- | --- |
| еда | `cm_auto_food_location_enabled` / `_excluded` — **обычная переменная локации** | да, `set_variable` в области локации |
| RGO | `cm_auto_expand_rgo` / `_excluded` / `cm_mass_auto_expand_rgo`, там же | да, так же |
| **здания** | **пофлажковой записи на локации нет вовсе** | — |

У зданий выбор игрока — это **типы зданий, глобально**:
`cm_auto_build_building_types_enabled`, список на стране, набираемый со страницы
CMM (`cmm_build_list_bool_list`, настройка `cm__auto_build_list`). Локация в
`cm_location_can_auto_build` не хранит ничего своего — тригер выводится из
«тип отмечен» плюс «нет устаревшего предшественника»
(`cm_auto_build_triggers.txt:87`). То есть модель CM для зданий — «отметь тип,
я построю везде, где мне позволено», а не «отметь место».

**Значит «одна ссылка, которая нажмёт все ссылки» для зданий нажимать нечего.**
Остаётся очередь без гейтов, описанная выше: `cm_stage_ungated_candidate`
принимает **и локацию, и тип здания**, что и есть нужная пара. На ней же
построена собственная Auto Build у CM.

**И одна оговорка, которая меняет объём работы: очередь одноразовая.**
Диспетчер чистит её каждый месячный импульс, поэтому это не флажок, который
поставил и забыл, — сложенное надо складывать заново каждый месяц, своим
листовым действием на `cmf_monthly_human_country_pulse`. Зато список, который
надо складывать, у `where_to_produce` уже есть: **это сам план**, пара
«локация + здание» на каждую строку.
