...
**Разбор его расчёта там же, и это следующая работа**: он считает по
**определению провинции**, а не по локации; вход меряет **долями рецепта**, а
не «есть/нет товара»; берёт **лучший способ**, не текущий; и своё РГО локации
считает полным покрытием. Наш `_plan_right_*` не делает ничего из трёх. Сверять
с картой, стоящей теперь рядом — она эталон, и его слово: «если у нас с ним
будет разница — я решу, что у него правильно».
