...
окне плана, ручным этапом после «Пересчитать», **переезд между провинциями**.

**Первая версия мерила и меняла пустоту, и это стоит помнить.** Выгода — свойство
**провинции**: `_g<n>` спрашивает `province_definition = {
any_location_in_province_definition = { raw_material = goods:X } }`, и все
локации провинции платят товару поровну. Обмен внутри провинции не может дать
ничего, а числа того зонда (18 244, 2.1 %) вдобавок сравнивали городскую выгоду
с сельской. Разбор — [`investigations/plan_gaps.md`](investigations/plan_gaps.md).

**И «Расширить» перестала стирать плановую квоту**: `_ext_quotas` клал
редакторское `_eq<i>` в обе стороны, и арифметика долей до доливки не доезжала.
Теперь незамороженный товар идёт по квоте `_plan_set_quota` для всей земли
Z = X + Y.
