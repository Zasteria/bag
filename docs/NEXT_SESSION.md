...
## Вторая версия закрыта, работа возвращается в первую

**2026-09-12: `where_to_produce_v2` построен и удалён в тот же день**
([`archive/wtp2_failed.md`](archive/wtp2_failed.md)). Править первую версию, а не
строить рядом новую; чужой инструмент, который просят «взять», — копировать
файлами, а не воспроизводить формулой.

**С чего начинать:** он играл с модом и пришёл со списком — логика, интерфейс,
удобство. Списка в документах нет. **Спросить у него, не выдумывать.**

## Открытый вопрос: пересборка после двух dev-сборок

Закоммиченное дерево чисто, но `refresh.py` 2026-09-12 после пересборки сказал
«`bag_wtp_src4` читается и никто его не ставит» (`bag_wtp_generated_diag.txt`),
**а ручная пересборка показала обратное** — оба файла согласны. Номера `_src<k>`
это порядок папок в `reference/mods/`, и dev-сборки их сдвинули. Измеряется без
игры: `generate.py`, затем `check_script.py mods/where_to_produce`, затем
`git checkout --`. Проверку в той сессии оборвали.
