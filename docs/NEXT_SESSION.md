...
## Работа: `where_to_produce` — раздача принята, дальше перетасовка

**Одиннадцатый прогон 2026-09-08 закрыл раздачу.** Его слова: «Кажется
постановка всего что надо наконец отрабатывает как положено». Земля 1380 из
1380, обе стороны полные, доли 19/55/74, 36 товаров из 37 остановила своя
квота. Остаток «−1» у восемнадцати городских измерен и это остаток от деления:
544 потолка на 528 комнат.

**Открытый вопрос, который он задал сам: есть ли конечная перетасовка.** Нет.
`_edit_reshuffle` торгует только теми зданиями, что долив поставил после «−1»
(на свежем плане `rounds=0`). И план не оптимален: раздача даёт товару его
лучшее **из оставшегося**.

**Первым делом — зонд, а не механизм**: сухой проход по парам одной провинции,
который считает выгодные обмены и их суммарную прибавку, ничего не двигая. Один
прогон скажет, стоит ли настоящий. Что для этого нужно построить и почему это
дороже, чем кажется (`_fillg` — одна переменная на локацию, а локация держит
три-четыре домика), — **[`investigations/plan_gaps.md`](investigations/plan_gaps.md)**,
раздел «перетасовка внутри провинции».
