...
## Работа: `where_to_produce` — сводка и сельский делитель, нужен прогон

**Построено 2026-09-08, в игре не было.** Две правки в раздаче и одна в окне
сводки, всё в одном прогоне.

1. **Сельское место считается только под своё здание.** «Деревня = товар»:
   украшения, инструменты и оружие входили в сельский делитель через 284 места
   торговой деревни, которых у них нет. 21 участник вместо 18, сельская доля 42
   вместо 50. Теперь `_ngr` считает то же, что пускают ворота
   `_plan_can_rural_<n>`.
2. **`_pqt`/`_pqr` больше не вычитают РГО со стороны, которой у товара нет.**
   Шерсть: сельский потолок был 22 при доле 10.
3. **Сводка**: «С РГО», «Домики», прочерк вместо нуля там, где стороны нет, и
   четыре столбца доли вместо одной — доля, доля −РГО, доля город, доля село.

**Просить у него**: «Пересчитать» на северной Германии и «Диагностика».
Ждём: доля город 12, село **50** (было 42); в столбце «Доля» ровно три числа —
12, 50 и 62; у бумаги и пива прочерк в «Село», у шерсти — в «Город»; шерсть
`доля город — · доля село 18`; рыбацкая деревня 42 из 42.
