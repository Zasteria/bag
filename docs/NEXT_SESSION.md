...
## Работа: `where_to_produce` — скидка за РГО внутри дележки, нужен прогон

**Построено 2026-09-08, в игре не было.** Девятый прогон принял всю таблицу и
всю ровность, но земля встала на 1121 из 1380: доля решала
`Σ min(земля, X) >= комнаты`, а строил товар `X − свои РГО`, и 284 РГО этой
земли оставляли ровно столько комнат пустыми.

1. **Скидка считается внутри дележки**: `Σ min(земля, X − РГО) >= комнаты`.
   Тогда каждый кончает на X **считая РГО** — его же мерка равномерности — и
   земля заполняется. Остаток скидки, не влезший в город, уходит в село
   (`_rgl<i>`, считается между двумя дележками).
2. **Комнаты берутся все, включая занятые грамотами.** Грамота ставит домик
   того же товара, и потолок товара его накрывает; вычет считал их дважды.
3. **Сухой круг снова поднимает долю** — как страховка от спора за общее
   здание, не как формула.
4. **Рыба уходит из корзины**, как шёлк: `_ng` — сумма своих сторон. И прочерк
   в столбце РГО у товаров, у которых РГО не бывает.

**Просить у него**: «Пересчитать» на северной Германии и «Диагностика».
Ждём: доля город **20**, село **55**, «Доля» — три числа 20/55/75; земля
близко к 1380; строки рыбы нет вовсе; у готовых товаров прочерк в «РГО».
