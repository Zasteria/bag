...
**Зонд построен 2026-09-09, в игре не было.** `_diag_shuffle` считает верхнюю
границу: на товар и провинцию — лучшая выгода, какую он там мог бы получить,
минус то, что он получает. Ничего не двигает, живёт только под «Диагностикой»
(два обхода провинции на товар — сорок тысяч заходов в локацию). `tools/diag.py`
печатает это одной строкой «коротко»: потолок прибавки, в процентах от нынешней
выгоды, сколько домиков стоят ниже своего лучшего места и какая доля этого
приходится на города с грамотой.

**Читать по нему так**: граница мала (проценты) — механизм строить незачем,
вопрос закрыт одним нажатием. Велика — строить, и тогда цена расписана в
**[`investigations/plan_gaps.md`](investigations/plan_gaps.md)**: `_fillg` —
одна переменная на локацию, а локация держит три-четыре домика.

**Тем же заходом**: у товара, недобравшего до потолка, в столбце «Доля −РГО»
дописано `#R −N#!` (`SelectLocalization`, форма из `interfaces_l_english.yml`).
Его просьба: «нельзя чтобы это терялось информационо».
