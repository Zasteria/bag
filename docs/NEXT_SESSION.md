...
## Работа: `where_to_produce` — резервация построена, нужен один прогон

**Он выбрал порядок, 2026-09-07: сначала резервация, потом перетасовка.**
Резервация построена в тот же день и **в игре не была**; всё про неё —
**[`investigations/plan_reservation.md`](investigations/plan_reservation.md)**:
правило, четыре места в коде, что оно должно дать в числах и по какому признаку
его снимать. Перетасовка описана там же, в конце, и берётся **после** прогона
первой.

**Просить у него**: «Пересчитать» на северной Германии и «Диагностика», логи
через `python3 tools/which_build.py <папка логов>`. Ждём `stone` 45 → около 60 и
`salt` 36 → около 47 в сводке, минус один-два домика у товаров, стоявших на 61,
и неподвижные `fish`, `naval_supplies`, `iron`. **Новые числа `owt`/`owr`** на
строке товара говорят, скольким локациям он хозяин: пустые — правило не
защитило никого и снимается; десятки при неподвижной сводке — правило работает,
а связало товар что-то другое.
...
Ни один из трёх не начинать раньше предыдущего, и **первый ждёт своего прогона**.
Один прогон на пункт: «Пересчитать» на северной Германии и «Диагностика» — этого
хватает, чтобы сказать, работает правка или снимается.
