...
**Житницы CM план обходит стороной**: локация с `cm_auto_food_location_enabled`
не получает ни домика, ни грамоты и её комнаты не идут в счёт мест — но из
списка не пропадает, её РГО считается как раньше, и выгода провинции от неё не
меняется. Кнопка житницы стоит в ряду локации там же, где её предлагает CM.
...
здания вне плана и только их. Разбор всего —
[`investigations/wtp_integration.md`](investigations/wtp_integration.md).

**Шаг 8 — одна отмашка и ничего периодического**, это его требование. Строка
`CM found/were_on/touched` говорит, что сделало последнее нажатие; `found=0` —
«в группе нечему стоять», а не «кнопка не работает».
