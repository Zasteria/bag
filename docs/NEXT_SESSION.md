...
**Живое — одно, и оно требует его решения.** Побережий 42, в них 126 комнат, и
`fish` с `naval_supplies`, которым больше некуда, держат из них 38; остальные 88
у товаров с четырьмястами альтернативами. Причина названа: доля рыбы (36 − 21
РГО = 15) держала её ровно тогда, когда побережье разбирали. Резервация по
стеснённости подняла бы обоих до 42, то есть ровно на общий уровень, ценой ~46
комнат у массовых товаров. Разбор —
[`investigations/plan_gaps.md`](investigations/plan_gaps.md). **Не построено.**
