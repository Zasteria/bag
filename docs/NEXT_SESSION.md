...
**Перетасовка построена 2026-09-09, в игре не была.** Кнопка «Перетасовать» в
окне плана, ручным этапом после «Пересчитать». Зонд намерил её потолок в 2.1 %
(из них 86 % заперты в грамотах), владелец: «даже если прирост 2 процента —
сделаем». Разбор — [`investigations/plan_gaps.md`](investigations/plan_gaps.md).

**И «Расширить» перестала стирать плановую квоту.** `_ext_quotas` клал
редакторское `_eq<i>` в обе стороны, и вся арифметика долей, выправленная
2026-09-08, до доливки не доезжала. Теперь незамороженный товар идёт по той
квоте, что `_plan_set_quota` посчитал по всей земле Z = X + Y — то есть ровно к
тому плану, каким он был бы на Z целиком.

**Просить у него один прогон:** «Пересчитать» → «Перетасовать» → «Диагностика»,
и отдельно «Расширить» на соседнюю землю. Ждём: домиков ровно столько же,
`EDIT shuffle` с ненулевыми `swaps`/`gain`, `moved_on_old=0` у доливки.
