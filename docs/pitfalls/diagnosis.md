...
- **Архивирование ни при чём.** `docs/archive/*.md` стоит в `SOURCES` у
  `kb.py`, и вынесенное туда находится поиском наравне с остальным. Проверено.
- **А вот `in_game/gui/*.gui` не индексировалось ничем.** `kb.py` берёт `docs/`,
  `code.py` брал только генераторы — а пять рукописных окон несут 516 строк
  комментариев, и каждая из них чем-то оплачена. Правило про `§` лежало там с
  первой сборки и было недостижимо: окно — 4 000 токенов, его никто не читает
  ради одного абзаца. **Теперь `code.py` индексует сами комментарии**, и
  «coloured … literally» находится с первого запроса.
- **И `CLAUDE.md` обещал `--map` за «~1 400 токенов»**, тогда как на деле
  `kb.py --map` — 4 000, `code.py --map` — 13 000. Сессия, поверившая цифре,
  тратит вдесятеро. Исправлено.

**Но три отказа из четырёх в ту сессию были не про доступность.** Правило про
`hbox`, который считает себя по детям, стояло в файле, который я в тот момент
правил. Имя кнопки я утверждал по памяти, хотя `CLAUDE.md` запрещает это прямым
текстом. «В игре нет такого символа» — вывод из одного поиска, тогда как та же
страница предупреждает не заключать об отсутствии по одному месту. **Индекс
чинит то, что было недостижимо; он не чинит то, что не было прочитано.**
