...
- **Значка нет — берите виджет игры, а не другой значок.** `✔` пришёл пустым
  квадратом, и в локализации самой игры нет ни одного такого символа, так что
  следующий был бы догадкой. `checkbutton_round_alt`
  (`shared/buttons.gui:1973`) рисует оба состояния из текстуры и берёт состояние
  из `down = "[...]"`.
