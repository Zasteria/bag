...
- **Скрытая ячейка ломает столбики.** `ignoreinvisible = yes` с `visible` на
  ячейке даёт рваные строки разной длины. Упаковать их нельзя (нужен datamodel,
  а `flowcontainer` с ним роняет игру и `fixedgridbox` рисует ячейки друг на
  друге) — значит выравнивать: ячейка стоит всегда, `visible` висит на её
  органах, невозможный товар оставляет ровный пробел.
