...
**Шапка окна не тянется за окном без `layoutpolicy_horizontal = expanding`.**
`window_header_alt` держит ширину своего типа, что бы ни делало окно вокруг, и
все пять окон `where_to_produce` были без этой строки. Владелец сообщал об этом
трижды — «злоебучая шапка окна снова не расширилась вместе с окном», 2026-09-06, —
и каждый раз это читалось как загадка вёрстки. **Собственные окна игры несут её
все**: `multiplayer_lobby.gui`, `automation_lateralview.gui`. Смотреть, как игра
объявляет тот же тип, дешевле, чем гадать.
