...
**Чинить в двух шагах:** размер задать по самой широкой строке, и задавать его
**в одном месте генератора**, а не цифрой в каждом окне — иначе это вернётся в
шестой раз. **Не проверено:** можно ли снять `allow_outside` с корня окна —
крестик стоит на `position = { -5 5 }` от правого края и, возможно, держится
именно на нём. Проверять до правки, а не после.
