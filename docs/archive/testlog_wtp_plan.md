...
**2026-09-04 — `where_to_produce`, откат тоже упал при открытии редактора. И это
назвало причину.** «снова вылет при открытии редактора».

- **Откат грузится** — краш на загрузке ушёл. Но редактор по-прежнему убивает
  игру, и это на сборке, которую я объявил «последней рабочей».
- **Она рабочей не была.** `a55e14b` = `c14aa0f` по файлам мода, а `c14aa0f`
  **ни разу не запускалась**: следующая сборка была собрана поверх неё. Рабочими
  были `59e1c61` и `8808561`.
- **И в `c14aa0f` появился `flowcontainer` с `datamodel`.** Ноль таких виджетов —
  окно открывается (2 сборки); один — краш (4 сборки). Совпадение полное. Карта
  переменных, флаги, `ScriptValue` в локализации и текстиконки — все ни при чём,
  три отката были впустую.
- **Ванилла так не делает нигде.** Ни одного `flowcontainer` с настоящим
  datamodel во всей игре: у двух её `wrap_count`-выборок дети написаны буквально,
  а где datamodel есть — это `DataModelRepeatedItem(N)`, счётчик, не список.
  Сетка **из списка** — это `fixedgridbox`, все 138 раз.
- **Исправлено на `fixedgridbox`** (`addcolumn`/`addrow`/`datamodel_wrap`/
  `flipdirection`), и `check_script.py` теперь отказывает такой паре — проверено
  поломкой нарочно.
...
**2026-09-04 — `where_to_produce`, счётчик у товара собран без карт вовсе.**
Сборка, не прогон. Карту переменных закрыли совсем: `docs/pitfalls/interface.md`.

- **Выборка товаров — 47 написанных ячеек в `bag_wtp_edit_window.gui`**, а не
  datamodel. Иконка товара — текстиконка `@iron!` прямо в локализации, число —
  `ScriptValue('bag_wtp_show_pn<n>')`, кнопки зовут именованные
  `bag_wtp_pick_plus_<n>`. Ни одна часть не нова: всё это окно уже где-то рисует.
- **Знак у числа вместо цвета**, как он и просил: «+» — есть куда поставить ещё
  один домик, «✖» — некуда, и подсказка называет причину из двух («стоит уже
  везде, где эта земля умеет» / «места есть, но там только последние домики и
  домики грамот»).
- **Знаки пересчитываются после каждого нажатия**, а не только при открытии.
- **Убрано мёртвое**: `_edit_pool` со своим `_edit_fill_pool`, мостики
  `bag_wtp_edit_plus`/`_minus`/`_pick`, которые несли товар как scope, и
  `bag_wtp_edit_set_good`, который их переводил в номер.
- **Не проверено:** открывается ли окно (после трёх крашей это первый вопрос),
  что показывают знаки, и делает ли «+1» ровно один домик.

**2026-09-04 — `where_to_produce`, число у товара собрано заново, на флагах.**
Не прогон, а сборка после отката: владелец сказал «Пробуй».

- **Ключ карты — флаг, а не объект базы.** `key = flag:clay` в скрипте,
  `MakeScopeFlag(Goods.GetKey)` в интерфейсе. Это форма CMF слово в слово, и этот
  мод уже читает такую карту сам — `"global_variable_map(cmm|flag:...)"` в старом
  чтении выпадающих списков.
- **Оба класса того, что могло уронить, закрыты.** `.IsSet` перед каждым
  `.GetValue` в `visible`, `GetValueWithDefault` в локализации — метода на
  отсутствующей записи нигде не осталось.
- **И предохранитель.** Тумблер «показывать число домиков в редакторе» на
  странице мода, по умолчанию включён; ячейки спрашивают его первым. Если
  крашнет снова — выключить там, и редактор работает: страница настроек окна не
  открывает. Третий краш не должен стоить сессии.
- **Не проверено:** ровно это всё. И цвет: `§G…§!` в подсказках мода работает, в
  обычном `text_single` вне кнопки — ни разу.

**2026-09-04 — `where_to_produce`, число у товара уронило игру. Откачено.**
Открытие окна редактора убивало игру, дважды подряд. **В логах нет ничего**: ни
ошибки разбора, ни строки мода — записи просто обрываются. Краш нативный.

- **Откачена вся правка целиком, а не её часть.** В ней было три новых механизма
  сразу, и выяснять, какой из них, ценой ещё одного краша — не то, за что можно
  платить его сессией.
- **Названо, чем сборка отличалась от проверенного.** CMF держит свои карты
  **всегда на флагах**: пишет `key = scope:setting`, где scope пришёл из
  `MakeScopeFlag(...)`, и читает
  `GetVariableFromGlobalVariableMap(name, MakeScopeFlag(...))`. Ни одной карты по
  объекту базы. Моя сборка ключевала по `goods:clay` и читала
  `Goods.MakeScope` — единственное расхождение с тем, что CMM рисует каждый кадр.
- **`Goods.GetKey` есть** (проверено по его же дампу `data_types`), так что флаг
  из строки списка получить можно, и путь к числу открыт. Не построено: после
  двух крашей это решает он, а не я.
