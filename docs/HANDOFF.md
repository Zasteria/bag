...
генератор и Вестфалия целиком есть. **Перекрытие проверено в игре и работает**:
отдельный мод, стоящий после базового, переопределяет уже определённые им ключи
локализации — это был главный риск, и он снят. Осталось получить русскую
локализацию самой игры — она теперь в `reference/`, и `tools/term.py` спрашивает у неё,
как игра называет своё понятие (advances — «Улучшения», а не «достижения»).

Сделано: все 5 правил игры, названия и описания 45 образуемых стран мода
(остальные 93 берут имя из самой игры и по-русски были всегда), три большие
системы целиком — Дунайская монархия, Дунайский вопрос, Австрия, — Вестфалия
целиком, названия Ломбардии и общий файл `nd_event_guards`. 1876 ключей.

Порядок дальнейшей работы записан в `mods/nd_ru/priority.txt`: Европа, от крупных
стран к мелким, потом остальной мир. `tools/scope.py --plan` печатает остаток по
каждой стране; рабочий набор страны — её названия плюс полное событие образования.
Осталось по Европе около 6000 ключей (26 тыс. слов), что примерно равно одной
сессии такого размера, как та, в которую всё это было сделано. Весь мод целиком —
664 тыс. слов, около двадцати семи таких сессий: на подписке Pro это не окупается,
и объём надо резать по приоритету, а не пытаться взять всё.
