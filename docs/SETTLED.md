...
| Why does the plan put glass in villages and never in a town? | **Because most of those «towns» are villages the player ticked into towns, and a guild is `town = yes`.** `rural_glassmaker` is `rural_settlement = yes, town = no`, `glass_guild` is `town/city/megalopolis = yes`; both carry the *identical* sand-in-market condition and it passes — the villages of those same markets all took the rural building. **So the market gate was never it**, measured on twenty goods at once: cloth, tools, pottery, jewelry and sixteen more, none with any market condition, each won a town method on the same 3 of 17 town-side locations. The tick moves a location to the plan's town side; it cannot move its rank in the game, and `can_build_building` answers about the rank. | TESTLOG 2026-09-02 |
