...
| [`where_to_produce`](../mods/where_to_produce/CLAUDE.md) | twenty-three loads in: a good or a whole urban right, a window each, live re-ranking, three ages on every row, two «Считать» buttons | the one `error.log` line, and everything of the twenty-fourth load |
