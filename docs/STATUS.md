...
| [`where_to_produce`](../mods/where_to_produce/CLAUDE.md) | **the plan works, all eight measured faults in its formula are closed, and seven of the eight are confirmed in game** — [`investigations/plan_gaps.md`](investigations/plan_gaps.md) is the record. The eighth was mine and the run caught it: gating charters on their unlocking advance left Münster one charter of thirteen and every town took it, so the plan uses `potential` as the window does and the advance is reported instead. «Диагностика» + `mods.bat → 8` reads the report for you | **one press, and it is about size** — the ladders were rebuilt for a large ground and 48 locations answered identically to the old build; northern Germany, 233 locations, «на конец» |
