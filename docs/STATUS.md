...
| [`where_to_produce`](../mods/where_to_produce/CLAUDE.md) | **the plan works and the formula has two regimes** — on a small ground the quota binds and the charters shape the answer; on a realm-sized one the quota binds nothing and the bands are the whole allocator, which is why cloth had 108 buildings and cannons 2. The open ladder now deals leftovers by each good's own best, and there is a hand weight per good for the disagreements a formula cannot settle. [`investigations/plan_gaps.md`](investigations/plan_gaps.md) is the record | **the relative open ladder, the hand regulator and the plan's own diff** — one press on a large ground, checklist at the end of `plan_gaps.md` |
