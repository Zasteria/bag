...
| [`where_to_produce`](../mods/where_to_produce/CLAUDE.md) | six loads in, the results window works; a row is a whole province now and the window's overflow is fixed, both untested | whole provinces instead of one owner's half, the goods count, the window inside its frame |
