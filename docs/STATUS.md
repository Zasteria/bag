...
| [`where_to_produce`](../mods/where_to_produce/CLAUDE.md) | **rolled back on 2026-09-02 to the build the thirty-sixth run praised** — «вау, оно кажется даже адекватно работает». The per-province answer, four ways to frame the ground, three ages a row, and the whole-map plan on the settled formula: quota, province divisor, mandatory rights. Everything built above it that day is out of the tree, findings kept in `investigations/plan_formula.md`. **The job is one probe for one symptom** — the plan will not put glass in a town (`pitfalls/diagnosis.md`) | the three faults the thirty-sixth run named, the charter spam, the per-tier sweep budget, the icon spacing |
