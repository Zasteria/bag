...
| [`where_to_produce`](../mods/where_to_produce/CLAUDE.md) | **rolled back on 2026-09-02 to the build of the thirty-eighth load** — the four tests he called «большой рывок», where the spread of goods over a whole ground first looked as he had meant it. The plan is an optimisation with a covering constraint; everything built above that build is out of the tree, its findings kept in `investigations/plan_formula.md`. **The job is one probe for one symptom** — the plan will not put glass in a town (`pitfalls/diagnosis.md`) | a big ground does not finish (the round guard), the province ceiling setting, the charter spam, the icon spacing |
