...
| [`where_to_produce`](../mods/where_to_produce/CLAUDE.md) | **Шаги 0–6 закрыты и видены в игре** тремя прогонами на северной Германии (416 локаций, 1380 зданий на 1380 мест). **Полосы выгоды не решают «сколько»**, товар останавливает **своя квота**, и план ровен настолько, насколько земля позволяет: считая РГО, девятнадцать товаров на **60…65** | **Два котла, город и село; в сельском считаются только свои здания** — товар, у которого в селе одна деревня, в делитель не входит, и в сводке у него прочерк ([`plan_rural_pool.md`](investigations/plan_rural_pool.md)). **Сводка перестроена 2026-09-08**: четыре столбца доли и прочерки вместо нулей — принято прогоном. **Скидка за РГО считается внутри дележки и ждёт прогона**: доля искала X по `Σ min(земля, X)`, а строил товар `X − РГО`, и 284 РГО оставляли 259 комнат пустыми ([`plan_rural_pool.md`](investigations/plan_rural_pool.md)). **Дальше: этот прогон, потом перетасовка внутри провинции и шаги 7 и 8 (Glorp UI и Construction Manager).** **Windows have their own checklist**, [`pitfalls/windows.md`](pitfalls/windows.md); остальные правила — [`archive/wtp_brief_rules.md`](archive/wtp_brief_rules.md) |
