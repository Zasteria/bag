...
| [`where_to_produce`](../mods/where_to_produce/CLAUDE.md) | eighteen loads in: a good or a whole urban right, a window each, live re-ranking, and both ages on every row | the fed-first rule, the two-slot buildings, the zero-bonus filter for rights, and the general/unique split |
