...
| [`where_to_produce`](../mods/where_to_produce/CLAUDE.md) | **the plan works, and a finished plan can now be edited in place** — one building a press into the location where it costs least, with its own good picker, a save slot and a changes list. The charter ladder's last band carries the quota now, which is what had one charter taking a fifth of the towns and two taking one each. [`investigations/plan_gaps.md`](investigations/plan_gaps.md) is the record | **the editor, the even charters and the relative open ladder** — the charters show on Westphalia, the ladder needs a large ground; checklist at the end of `plan_gaps.md` |
