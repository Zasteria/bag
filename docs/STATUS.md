...
| [`where_to_produce`](../mods/where_to_produce/CLAUDE.md) | **at the build of the thirty-eighth load** — the four tests he called «большой рывок» — with the diagnosis built on top of it and never loaded. «Диагностика» writes everything the mod knows into `debug.log` in one press; `mods.bat → 8` puts it in the clipboard. **The next step is one run and reading it**: the protocol and what every branch of the answer means are in `TESTLOG.md`, written down before the run | the diagnosis itself; a big ground does not finish (the round guard); the province ceiling setting; the charter spam |
