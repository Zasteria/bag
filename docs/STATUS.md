...
| [`where_to_produce`](../mods/where_to_produce/CLAUDE.md) | **the plan works and he has seen it** — 192 buildings in 192 rooms, charters 5–6 each. **The editor is a window of its own**: three slots, a cell per good, a changes list beside it. Five silent interface faults cost that and all are written down ([`pitfalls/interface.md`](pitfalls/interface.md)) | **«+1»/«−1» work and the round trip is not an undo** — the plan drifts by one building of two other goods (`TESTLOG.md`). **Steps 1, 2 and 2в of eight are closed and confirmed in game**, and one wrong change reverted with them: **everything the editor holds is the editor's alone, a fresh plan reads none of it**. **The menu rebuild is built, run and accepted, 2026-09-06** — «почти идеально»: three tabs, four buttons that only open windows, every setting inside its function's window. **Two rounds of fixes followed**: the buildable tick deleted, «Выбрать мою землю» added, the stamp and the essays out of the windows, and four layout faults, chief among them **an `hbox` with a written width measuring 132 short**. **The editor's picker is all 47 goods now**, uniform: aligned and packed both read as «страшный набор столбиков». **One column drift in the rights window is unexplained** and deliberately not guessed at. **Two rules of his**: no descriptive text in a window header, technical things on the technical tab. [`investigations/wtp_menu_rebuild.md`](investigations/wtp_menu_rebuild.md). Order, and the rules it implements: [`investigations/wtp_practice_plan.md`](investigations/wtp_practice_plan.md), [`wtp_editor_design.md`](investigations/wtp_editor_design.md) |
