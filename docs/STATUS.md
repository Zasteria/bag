...
| [`where_to_produce`](../mods/where_to_produce/CLAUDE.md) | **the plan works, and a finished plan is edited in a window of its own** — three save slots, the ground's goods as icons, «−1»/«+1» beside each, and a changes list in a second window; the settings page keeps one button that opens it. The charter ladder raises its ceiling one town at a time and runs every band at each height, which is what had two charters stuck at three towns while seven reached six. [`investigations/plan_gaps.md`](investigations/plan_gaps.md) is the record | **the three windows, the even charters and the relative open ladder** — no `.gui` here has ever been drawn, the charters show on Westphalia, the ladder needs a large ground; checklist at the end of `plan_gaps.md` |
