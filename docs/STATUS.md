...
| [`where_to_produce`](../mods/where_to_produce/CLAUDE.md) | **the plan works, and five measured faults in its formula are open** — [`investigations/plan_gaps.md`](investigations/plan_gaps.md) is the job. The covering pass ran last and placed nothing on full ground, so `stone` got zero buildings and the weaponry charter landed where it scores zero; the scarcity tiers place 5 buildings of 84; a deep recipe cannot score anywhere by construction. «Диагностика» + `mods.bat → 8` reads the report for you | one run owed on everything fixed 2026-09-03; the tier-vs-band inversion and the deep-recipe bias are the owner's call |
