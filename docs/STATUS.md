...
| [`where_to_produce`](../mods/where_to_produce/CLAUDE.md) | fifteen loads in; works for a single good. The rights window did not parse on its first load — two byte order marks — and the buildable filter turns out never to have filtered | the rights window, the buildable tick, the folded pickers, the age filter |
