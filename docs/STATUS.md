...
| [`where_to_produce`](../mods/where_to_produce/CLAUDE.md) | twenty-seven loads in and the per-province answer is trusted: a good or a whole urban right, four ways to frame the ground, three ages on every row. the plan is an optimisation with a covering constraint and the owner has seen it work: «большой рывок». **One symptom is open and four theories about it were all wrong — the next move is a probe, not a cause** (`pitfalls/diagnosis.md`). `investigations/plan_formula.md` is the derivation and must be read first | the per-tier sweep budget, the `n/m` fit count on a ranking row, the icon spacing |
