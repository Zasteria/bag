...
| [`where_to_produce`](../mods/where_to_produce/CLAUDE.md) | **at the build of the thirty-eighth load, and the symptom that outlived four theories is measured**: the «сделать городом» tick moves a location to the plan's town side and cannot move its rank in the game, so a guild never stands there and the granted charter comes out half-made (`SETTLED.md`). **What the tick ought to mean is his decision** — three readings in `NEXT_SESSION.md`, ask before building any of them. «Диагностика» + `mods.bat → 8` is how the numbers come out | the decision itself; a big ground does not finish (the round guard); the province ceiling setting |
