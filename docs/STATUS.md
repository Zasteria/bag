...
| [`where_to_produce`](../mods/where_to_produce/CLAUDE.md) | nineteen loads in: a good or a whole urban right, a window each, live re-ranking, both ages on every row, sorted by clicking a column | the two-slot buildings, the zero-bonus filter for rights, and the general/unique split |
