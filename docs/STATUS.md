...
| [`where_to_produce`](../mods/where_to_produce/CLAUDE.md) | **the plan works and he has seen it** — 192 buildings in 192 rooms, charters 5–6 each. **The editor is a window of its own**: three slots, a cell per good, a changes list beside it. Five silent interface faults cost that ([`pitfalls/interface.md`](pitfalls/interface.md)) | **«+1»/«−1» work and the round trip is not an undo** — the plan drifts by one building of two other goods (`TESTLOG.md`). **Steps 1, 2, 2в, 2а and 3 of eight are closed**, and one wrong change reverted with them: **everything the editor holds is the editor's alone, a fresh plan reads none of it**. **The menu rebuild is done, 2026-09-06** — three tabs, four buttons that only open windows, and the rule under all its fixes — **a box with room to spare divides it between its children** — is in `pitfalls/interface.md`. **Step 2а is confirmed by numbers, 2026-09-06** — `shuffle rounds=68 swaps=19 gain=1971`; a later run traded nothing and four counters were added to say why. **Step 3 is confirmed, 2026-09-06** (`EDIT scan strict=1`): «+1» takes its building from a good over its share. **Its victim rule changed once more and is unrun** — furthest above the share first, cheapest building only as a tie-break. **The changes window has no chronology and never can** — it is a diff; the press journal goes into `debug.log` at press time and `tools/diag.py` renders it. **Two rules of his**: no descriptions in a window header, technical things on the technical tab. Order and rules: [`investigations/wtp_practice_plan.md`](investigations/wtp_practice_plan.md), [`wtp_editor_design.md`](investigations/wtp_editor_design.md) |
